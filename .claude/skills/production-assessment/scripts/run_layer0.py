...
def gate_module_coverage() -> dict:
    ...
    FAIL-CLOSED on an unresolvable module (Issue 501). An unmeasurable floored
    module is a floor with no evidence, not a passing one. This used to `continue`
    on ``rate is None``, leaving ``_failures`` empty so the gate evaluated ``ok`` —
    which ``--require`` accepts. That is not hypothetical: Issue 368's multi-root
    ``--cov`` flattened ``clip_engine`` and ``preference`` into package "." and both
    floors read 0.0 for a YEAR while appearing configured.

    Note the deliberate asymmetry with ``_module_line_rate``, which returns None
    rather than guessing on an ambiguous suffix match. The resolver refuses to
    invent a rate; this gate refuses to pass without one. Same reading, opposite
    postures, and both are load-bearing — the same consequence-class split as
    Issue 524's ``_verify_rendered_output`` vs ``_source_duration_s``, where the
    identical ffprobe call is lenient about an unknown *source* and strict about
    an unknown *output*.
    """
    xml_out = ASSESS_DIR / "_coverage.xml"
    if not xml_out.exists():
    ...
    for mod, floor in MODULE_COVERAGE_FLOORS.items():
        ...
        if rate is None:
            failures.append(
                f"{mod}: NOT RESOLVABLE in coverage report (floor {floor:.1f}% unenforced)"
            )
            continue
        if rate < floor - 0.01:
