# dna — assessed 2026-05-31 (Wave 3)

Slice: `dna/__init__.py` (empty), `dna/brief.py`, `dna/builder.py`,
`dna/conflict.py`, `dna/embeddings.py`, `dna/identity.py`, `dna/profile.py`.
Wave 3 did NOT touch any file in `dna/` — `git log 84a7e9f..HEAD -- dna/` is
empty. The only Wave-3 surface that reaches `dna/` is `worker.anthropic_stream.
stream_and_emit`'s new `tools: list | None = None` kwarg
(`worker/anthropic_stream.py:36-76`); the implementation builds `stream_kwargs`
as a dict and only inserts `"tools"` when the caller passed a non-None value
(`worker/anthropic_stream.py:69-76`). `dna/brief.py:136-143` calls
`stream_and_emit(...)` with NO `tools=` argument, so the kwarg defaults to
None and is dropped from `stream_kwargs` entirely before the
`client.messages.stream(**stream_kwargs)` call — the docstring at
`worker/anthropic_stream.py:66-68` calls this out by name ("matches
`dna/brief.py`'s call shape"). The dna→stream path is unaffected by Wave 3.
This re-verification therefore confirms (a) the three carry-forward SEV2s and
the Issue-98 KISS cleanup from Wave 2 are still unchanged at HEAD, (b) the
Issue-84 audit cleanups (stale 2048-token docstring at `brief.py:7`,
asymmetric `# type: ignore[arg-type]` at `brief.py:153-157`, missing TTL-tier
cache breakdown in the token-log lines at `brief.py:144-150` and `:160-166`)
still reproduce at HEAD and remain blocked behind the SDK bump, and (c) no
new defects were introduced by Wave 3 in this slice. Callers traced:
`worker/tasks.py::_build_dna_async` (streaming + onboarding transitions),
`worker/anthropic_stream.py::stream_and_emit` (Wave-3 `tools` kwarg verified
backward-compatible with `dna/brief.py`'s zero-tool call), `worker/progress.py
::sync_emit` (per-token XADD).

## Findings

### Re-verified carry-forwards (Wave 1 → Wave 2 → Wave 3 unchanged)
...
- [SEV2] dna/brief.py:130-151 (concurrency & scale — sync stream on the
  ...
  `_build_dna_async` wrapped in `asyncio.to_thread` (worker/tasks.py), but when
  `task_id is not None` it calls `stream_and_emit`, which iterates the sync
  `client.messages.stream(...)` and routes every `content_block_delta`
  through `worker.progress.sync_emit` — a synchronous redis-py `XADD` per
  token (worker/progress.py, worker/anthropic_stream.py:78-86). At
  `max_tokens=2000` that's up to ~2k synchronous Redis XADDs per brief on a
  single worker threadpool slot. Bounded latency (Redis <1ms LAN) but each
...
- [SEV2] dna/brief.py:153-157 (resource lifecycle / Anthropic SDK —
  ...
  `list[dict]` shapes WITHOUT a type-ignore. Wave-3's `stream_and_emit`
  signature is `system: Any, messages: list, tools: list | None = None`
  (worker/anthropic_stream.py:36-45) — so the asymmetry is currently
  masked by `system: Any`; the moment the SDK bump to 0.105.2 tightens
  `_build_request`'s return shape or the wrapper narrows `system` to
  `list[TextBlockParam]`, the call site at brief.py:136-143 will fail mypy
  while brief.py:153-157 stays silently ignored. | fix: either mirror
...
- [cleanup] dna/profile.py:82-84 (KISS — split state-machine ownership) —
  ...
  complementary `awaiting_data → dna_pending` mutation in
  `worker/tasks.py` is still owned by the worker. Both transitions are
  correct under SQLAlchemy identity-map sharing (both `session.get` calls
  return the same instance inside the worker's `async with
  db.AdminSessionLocal()` block), but the split — one transition in
  `dna/profile.py`, the other in `worker/tasks.py` — is non-obvious. The
  next refactor that deletes the worker block thinking `create_draft` owns
  all transitions will silently break the `awaiting_data` entry path.
  | fix: prefer (a) move the `awaiting_data → dna_pending` bump into
  `create_draft` so the function owns ALL non-`active`/non-`dna_pending`
  entry transitions and the worker's state-machine line collapses to a
  comment, OR (b) add an inline comment at dna/profile.py:82 pointing to
  the worker site so the next reader sees the full state machine.
  Preference: (a) — it makes the `commit=False` caller contract simpler
  ("transition handled, just commit") and removes hidden cross-module
  coupling on the onboarding state.
...
- [cleanup] dna/brief.py:7 (Anthropic SDK — stale cache-floor docstring) —
  ...
  flat system block list. Gated on a deliberate decision rather than the
  SDK bump.
...
- [cleanup] dna/brief.py:144-150,160-166 (Anthropic SDK — missing TTL-tier
  ...
  a 1h-tier write (premium). Same gap in `worker/anthropic_stream.py:93-100`
  (Wave-3 unchanged) and the `cache` sync_emit path. | fix: blocked behind
  the 0.40 → 0.105.2 SDK bump (Issue 84 follow-up); once landed, extend both
...
### Wave-3 verification: stream_and_emit `tools` kwarg backward compatibility

- ✅ `dna/brief.py:136-143` calls `stream_and_emit(_ANTHROPIC, task_id,
  model=..., max_tokens=2000, system=system, messages=messages)` with NO
  `tools=` argument. `stream_and_emit` defaults `tools: list | None = None`
  (worker/anthropic_stream.py:44) and conditionally inserts the key
  (`if tools is not None: stream_kwargs["tools"] = tools` —
  worker/anthropic_stream.py:75-76). The final
  `client.messages.stream(**stream_kwargs)` call (line 78) therefore
  receives exactly the same kwargs it did in Wave 2 — no `tools=None`
  passthrough, no SDK shape change visible to this caller. Wave-3 dna
  brief streaming behavior is bit-for-bit identical to Wave 2.
...
- ✅ Prior SEV1 (`_build_dna_async` idempotency check-then-act) — still
  ...
  the transaction, re-check under the lock before any paid Anthropic /
  Voyage call, partial UNIQUE on `build_job_id` from migration 0008 as
  backstop, IntegrityError-on-commit treated as idempotent no-op.
...
- ✅ Streaming-path emission (Issue 86) — unchanged by Wave 3. Cache
  hit/miss + input tokens forwarded as `event: cache` before the first
  generated token via `message_start.usage` (anthropic_stream.py:110-124).
  Each `text_delta` forwarded as `event: token`; unknown delta types
  silently dropped (defensive against future SDK additions). Wave-3
  Fix A (the `tools` kwarg) is the only change to this file and does
  not affect dna's no-tools call.
...
clean — Wave 3 did not touch `dna/` (`git log 84a7e9f..HEAD -- dna/` is
empty) and the only adjacent Wave-3 change (`stream_and_emit`'s new
`tools` kwarg in `worker/anthropic_stream.py:36-76`) is backward-
compatible by construction: the dict-based `stream_kwargs` omits `tools`
when the caller doesn't pass it, and `dna/brief.py:136-143` doesn't pass
it. All prior BLOCKER/SEV1 findings remain structurally fixed. The
...
logs, obsolete `# type: ignore[arg-type]`) all reproduce at HEAD and
are correctly tracked as follow-ups gated on the SDK bump and a
