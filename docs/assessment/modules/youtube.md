...
Post-Wave-8 re-assessment. **Waves 5–8 did not touch any file under `youtube/`**:
Wave 8 shipped review-UI / insights-rebuild / walkthrough-gating (Issues 93, 94,
95, 100) entirely in `routers/`, `static/`, and `worker/`. The slice's state is
identical to the post-Wave-4 baseline. Scope: every Python file in `youtube/`
(`__init__.py`, `_http.py`, `_redis.py`, `errors.py`, `quota.py`,
`categories.py`, `oauth.py`, `analytics.py`, `data_api.py`, `ingest.py`).
Callers in `worker/` and `routers/` are other agents' slices and are referenced
only to verify how this module's functions are invoked.
...
- [SEV2] youtube/oauth.py:290 — `get_valid_access_token` still has no
  ...
  that calls `get_valid_access_token` (`sync_channel_catalog`,
  `refresh_youtube_analytics`, `sync_video_analytics`, every router that needs
  a fresh token). The module's graceful-degrade posture (quota → "try
  tomorrow", auth-error → drop row, ffmpeg → bounded timeout) is defeated by a
  single Redis blip — every near-expiry token path 500s during a broker
  outage. Carry-forward from 2026-05-29 / 2026-05-30 / 2026-05-31 (Waves 4–8
  did not land a fix; none of Waves 5–8 touched this file). | fix: wrap the
  `set()` in `try: acquired = ... except redis.RedisError as exc:` and either
  (a) fall back to lockless refresh (`acquired = True`; Google tolerates rare
  ...
  unavailable")`. Add a regression test that mocks `redis_client.set` to raise
...
- [cleanup] youtube/analytics.py:178 — `fetch_audience_activity` hardcodes
  ...
  mistake it for a real "noon" hour. Carry-forward from 2026-05-29 /
  2026-05-30 / 2026-05-31 (still present; Waves 4–8 did not touch this file).
  | fix: name it (`_HOUR_UNAVAILABLE_SENTINEL = 12`) and reference the model
  docstring, or document on the `AudienceActivity.hour` column that it is a
  fixed sentinel until hour-level data is available.

## Closed since last assessment

No new closures since the post-Wave-4 assessment — Waves 5–8 did not touch any
file in this slice. The Wave-4 closure (analytics retention purge + 30-day
cutoff + COMPLIANCE.md §2 documentation, Issue 75b) remains green and verified
below.
...
- **Wave-4 purge interaction with this module's writers.** The purge keys on
  ...
  (`analytics.py:260, 286, 295, 302, 304`). When a creator's daily Beat refresh
...
- **Issue 87 Shorts threshold.** `data_api.py:53` reads
  `settings.SHORTS_MAX_DURATION_S` (`config.py:103` default 180,
  `.env.example:71` documented "YouTube's official Shorts max (raised from 60s
  in Oct 2024)"). Matches YouTube's current spec; no hardcoded 60.
...
- **Tokens: only via decrypt()/encrypt().** `decrypt()` at `oauth.py:229,
  ...
  319-323`; `analytics.py:60, 83, 246`; `data_api.py:100, 124`;
...
- **Shared httpx singleton (`_http.client()`).** Lazy (`_http.py:24-29`) —
  ...
  per Issue 39). Reused at every Google call (`oauth.py:88, 94, 103`;
...
- **`yt-dlp` off by default**, guarded by `YTDLP_ENABLED`
  ...
  (`ingest.py:1-9`); present in `.env.example:63` with the correct warning.
...
| 3 Security & compliance | ok — tokens / isolation / revocation / Retry-After clean; analytics retention purge + 30-day staleness cutoff in place (Wave-4 Fix 3 / Issue 75b); §130 launch-gate checkbox closed |
...
## Module verdict
NEEDS-WORK — Waves 5–8 did not touch this slice, so the post-Wave-4 state holds:
the Wave-4 compliance closure (analytics retention purge + 30-day cutoff +
COMPLIANCE.md §2) remains green, and the same single SEV2 carries forward — the
unhandled Redis-down path in `get_valid_access_token` (`oauth.py:290`) that
turns a broker blip into 500s across every token-refresh path. One cleanup
(magic `hour=12` sentinel in `fetch_audience_activity`) also persists. Both are
bounded fixes; nothing in this module rises to BLOCKER or SEV1.
