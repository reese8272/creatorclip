...
│       ├── lib/                # api.ts (typed fetch) · queryClient.ts · brief.ts (+test) · taskStream.ts (SSE) · utils.ts · videosPoll.ts (shared ['videos'] refetch interval, Issue 369) · toolLayout.ts (tool-shell player widths, derived from viewport HEIGHT so a 9:16/16:9 player cannot crowd out its panel — Issue 389; note the ~14.39px root font-size trap documented there)
...
│       ├── components/editor/   # Timeline (+test) — waveform + synced playhead + cut overlays (Issue 188) · ShortFormEditor (the single-clip workspace, extracted from a 700-line Editor.tsx so Issues 390/391/392 conflict in different files — Issue 389) · LongFormEditor (real source player + drag-to-select create-clip + Your-clips group + per-clip export — Issues 307/372/373) · FullTranscriptPanel (searchable segment transcript, seek + Clip-this — Issue 372)
