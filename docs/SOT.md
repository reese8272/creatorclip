...
│   ├── camera_region.py        # (NEW Issue 430) cv2 temporal-variance camera-region detection for produced layouts (static chrome vs moving camera); fail-open gates keep plain sources byte-identical. Video-level rect = a majority-agreed component-wise median over 9 disjoint 60s windows (Issue 443); one detection over a whole runtime saturates the motion mask
│   ├── overlay_bands.py        # (NEW Issue 448) transient overlay-band detection (livestream superchats). A superchat is drawn ON TOP of the camera feed, so it lands INSIDE the camera region and the crop cannot remove it; masked at render by a time-gated split→crop→boxblur→overlay. Deliberately NOT part of camera_region.py: that pass's nine-window median is blind to transients by design (Issue 443), and these are the transients
...
│   ├── speaker_map.py          # (NEW Issue 420) PURE speaker→face mapping: per-shot greedy-NN face tracks, diarized turns (gap-merge/backchannel absorb), mouth-motion energy, weighted-vote mapping w/ margin-ratio confidence, fallback ladder speaker_cut→face_pan→static · `speaking_track_for_span()` (Issue 450) exposes the speaker→track mapping to the framing planner, which previously chose its seat with a largest-face vote uncorrelated with who is talking
