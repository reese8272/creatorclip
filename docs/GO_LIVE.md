...
| Do notifications actually reach the creator? | **Provision Resend**: create the API key, verify the sending domain, set `RESEND_API_KEY` + `EMAIL_FROM` + `NOTIFY_BACKEND=resend` in the VM `.env`, redeploy, and confirm the Issue-525 startup warning is gone. | operator | **OPEN (verified broken)** | **#529.** Measured on the running prod container 2026-08-18: `ENV=production`, `NOTIFY_BACKEND=console`, no `RESEND_API_KEY`, no `EMAIL_FROM`, and **17 `notification_deliveries` rows every one of which says `status='sent'` — none delivered**. `_send_console` logs and returns None while the row is committed `sent` before the send, so the database claims deliveries that never left the box. **No creator has ever received an email from this product.** Issue 525 made the failure visible (startup warning + `handled_by` on new rows) but cannot make anything deliver. **Blocks #28** — a friend who uploads today gets no email and the DB says one was sent. |
...
| Are all external APIs provisioned with `/health` green? | None — verified | build | GREEN (2026-07-29) | #25: `doctor.py --full` on the VM = **30 ok / 0 fail** (Anthropic, Voyage, Deepgram, R2, Stripe all live-verified), `/health` all-ok, key-leak grep over 30d app+worker logs = 0 hits; LLM E2E nightly green daily — **and that "green" means something as of 2026-08-20 (Issue 504)**: the nightly used to exit 0 with every test `skipif`-skipped if `ANTHROPIC_API_KEY`/`DEEPGRAM_API_KEY` expanded to `""`, so this cell could have been citing zero live calls. It now fails loudly on an empty secret and asserts a minimum *executed* (not collected) count per lane |
...
| Does connecting a YouTube channel actually import it? | None — fixed, deployed and live-verified | build → operator | **GREEN (2026-08-14)** | **The catalog sync imported nothing for ANY creator between 2026-06-24 and 2026-08-14.** `_FIELDS_PLAYLIST_ITEMS` omitted `snippet/resourceId/kind` while `list_channel_videos` filtered items on exactly that key; a `fields` spec returns only the properties it names, so every item was dropped and `sync_video_catalog` returned early. HTTP 200, no error, `"Synced N video(s)"` in the worker log. **Proven live** from inside the prod app container against the owner's channel: with the filter, `resourceId` keys are `['videoId']` and 0 of 5 items survive; without it, `['kind','videoId']` and 5 of 5 survive. Corroborated in the prod DB — every `origin=catalog` row was created 2026-06-01/02, zero in the seven weeks since. Fixed on `fix/catalog-sync-and-grounding-honesty` with a contract test that feeds each fixture through the real `fields` spec before parsing (the old fixture hand-wrote `kind`, and nothing referenced it). **This gates #28**: a friend who connects, syncs, and sees 0 videos cannot exercise the pipeline at all. **GREEN (2026-08-14).** Deployed as `1221fb8` (staging gate → prod, both green); the fixed `fields` spec was read back out of the RUNNING app and worker containers before testing anything. One real sync driven through the actual HTTP endpoint (`POST /creators/me/catalog/sync`, session cookie minted as `llm_harness.py` does — not a direct Celery enqueue, so auth, rate limiting and ownership stamping were all exercised), task `12430bde`. **Before → after** for creator `eb9af967`: catalog videos **0 → 21** (10 long / 15 Shorts); `video_metrics` with a non-null `engagement_rate` **0 → 21**; retention-curve points **0 → 2100**; and `GET /creators/me/data-gate` **0/0 `ready:false` → `{long_form_videos:6, shorts:15, shorts_ready:true, ready:true}`**. The worker log is the counterpart to the outage signature: `discovered 21 new video(s) from the uploads playlist` then `metrics written for 21 of 25 attempted` — the 4 misses are the upload-origin videos with no `youtube_video_id`, which the new honest counter correctly declines to claim (the old counter would have said 25). |
| Has the full pipeline run end-to-end on prod with real friends for 48h? | **UNBLOCKED 2026-08-14** — the catalog-sync row above is GREEN, so a friend can now connect, sync, and get a populated library — invite stands, but a friend cannot complete the journey until the fix ships | operator | **OPEN (in progress)** | #28. Unblocked: #24/#25/#26 are GREEN and billing cleared 2026-08-14, so this is now the **sole remaining hard blocker** for the friend beta. ⚠️ The invite is not the gate — the criterion is the *full pipeline exercised on prod over 48 h*, so the friend must actually upload and review, not just sign in. Two things to warn him about up front so they don't read as breakage: (a) the **"Google hasn't verified this app"** interstitial at consent, which is expected while #29 is unsubmitted; (b) in Testing publishing status Google issues **refresh tokens that expire after 7 days**, so he must re-connect YouTube weekly and any background analytics refresh will fail on day 8 ([Google docs](https://developers.google.com/identity/protocols/oauth2#expiration)) — this is independent of user count and is the real reason #29 matters, not the 100-user cap. Result + date: ________ |
...
**Stage A totals:** 35 gates — **20 GREEN · 5 CODE-GREEN · 10 OPEN · 0 RED** (recounted from the
rows themselves 2026-08-14, second pass). The count rose by one because the catalog-sync row was
ADDED this day, not because a status changed; it went OPEN → GREEN within the same day (found,
fixed, deployed as `1221fb8`, and live-verified: 0 → 21 catalog videos, gate `ready:true`). **Changed 2026-08-14: billing went RED → GREEN — the last RED on the
