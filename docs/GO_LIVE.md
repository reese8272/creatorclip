...
| Are the W1/W2 staging-verify residuals exercised? | **Make the `#228 rate-limit` leg prove its property, then re-dispatch** — the other two legs are done | staging-verify | OPEN (2 of 3 legs proven) | `.github/workflows/staging-drills.yml`. Run [31709917464](https://github.com/reese8272/creatorclip/actions/runs/31709917464) (2026-08-13) is green and **#284 flags-flip + #290 spend-trip are genuinely proven** (see rows above). **This row stays OPEN solely for the `#228 rate-limit` leg, which passed vacuously:** it asserts `all(c == 404 for c in codes[:first_429])`, and the daily render bucket is a persistent Redis counter, so on an already-spent quota the first probe is 429, `first_429 == 0`, and the guard degenerates to `all([]) == True` — the run logged `0 cheap 404 probes then 429 at request #1 (limit=60). PASS`. It proved "a 429 occurs", not "429 occurs only after the limit". Logged in `docs/OFF_COURSE_BUGS.md` (2026-08-13). Also: the 2026-07-03 red is superseded — its `AttributeError: module 'flags' has no attribute 'set_flag'` was fixed in `ca3305c` and pinned in #100 |
