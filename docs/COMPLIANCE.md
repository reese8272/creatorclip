...
| Source media | Raw video bytes (`videos.source_uri`) | Purged `SOURCE_MEDIA_RETENTION_HOURS` (default 72h) after ingest completion (`videos.ingest_done_at`), not upload time — see Issue 43 | Never stored longer than needed for processing. The video is retained through the render window (migration 0039) — rendering IS processing; auto-render runs within minutes of ingest. **Write path (Issue 395):** the browser writes source bytes directly to R2 via short-lived presigned URLs (15-min expiry, per-creator key namespace enforced at signature issuance); no new processor is introduced — R2 was already the storage processor. Incomplete multipart uploads (started but never completed) are auto-aborted by R2 after 7 days; the retention clock above starts at ingest completion exactly as before. |
...
| Rendered clips | 9:16 Short output (`clips.render_uri`, `clips.cleaned_render_uri`) | Until the creator archives or erases the video, erases the clip's video, or deletes the account | Stored in R2 (media bucket) at NON-creator-scoped keys (`clips/{clip_id}.mp4`, `_clean`, `_edit`), so deletion is by **per-URI enumeration** (`worker/erasure.py` — DB pointers ∪ deterministic constructed keys, which also reaches renders orphaned by the clean-confirm swap), never by prefix. Archive (Issue 446) purges all render variants while PRESERVING the clip + `clip_feedback` rows; `DELETE /auth/me` and `POST /videos/{id}/erase` use the same enumeration. Delete-protected by a short R2 Object Lock window on `clips/` (Issue 258): a refused delete is logged, the DB pointer is kept, and the retention sweep retries — reconciled with right-to-erasure (Issue 254). |
| **Poster frames (Issue 387)** | One 640px JPEG still per video (`videos.poster_uri`) and per rendered clip (`clips.poster_uri`), stored at `posters/{creator_id}/…` | **Until the row is deleted — this DELIBERATELY OUTLIVES the 72h source purge.** `purge_stale_source_media` does not touch `poster_uri`. Removed by row cascade and by the `posters/{creator_id}/` prefix sweep in `DELETE /auth/me`. | Derived artifact, not YouTube-origin data. **Why this differs from extracted audio, which IS purged at 72h:** the WAV is a lossless substitute for the entire source audio track; a poster is a **single lossy still** — on the order of one frame in ~30,000 for a 20-minute upload — from which nothing can be reconstructed. Its function is an **index entry**: the same data class as `videos.title` and `videos.duration_s`, which we already retain for the life of the row. The feature also *requires* it — the moment the source is purged is precisely when a creator most needs to recognise which row was which, so a poster that vanished with the source would go dark on exactly the rows it exists to rescue. Creator-scoped key so right-to-erasure reaches it via BOTH per-URI enumeration and the `posters/{creator_id}/` prefix sweep. (The former `clips/` gap is CLOSED — Issues 446/471: renders are enumerated per-URI.) **Posters survive video ARCHIVE by design** (Issue 446): archive is when a creator most needs to recognise a row to restore it; they are removed by permanent erase (`POST /videos/{id}/erase`) and account deletion. |
| **Waveform peaks (Issue 392)** | One BBC-`audiowaveform` JSON envelope per video (`videos.peaks_uri`), ~31 min/max pairs per second at 8-bit, stored at `peaks/{creator_id}/…` | **Until the row is deleted — this DELIBERATELY OUTLIVES the 72h source purge**, on the same basis as poster frames. Removed by row cascade and by the `peaks/{creator_id}/` prefix sweep in `DELETE /auth/me`. Like posters, peaks SURVIVE video archive (Issue 446) and are removed by permanent erase. | Derived artifact, not YouTube-origin data. **Why this differs from the extracted audio it is computed from:** the WAV is a lossless substitute for the entire source audio track — every word spoken, in full. A peaks file is an 8-bit **amplitude envelope at ~31 samples per second**, i.e. roughly 1/500th of the information rate of the 16 kHz PCM it came from, and it is strictly coarser than the `duration_s` + loudness data we already retain. No speech, music, or content of any kind can be reconstructed from it; it encodes only "how loud, roughly, at each moment". Its function is navigational — the same data class as a poster frame. **The retention asymmetry is load-bearing in the other direction too:** because peaks are computed FROM `audio_uri`, and that column is purged at 72h, a video ingested before Issue 392 whose audio has already gone can never receive peaks. `backfill_video_peaks` simply stops matching it, `has_peaks` stays false permanently, and the editor draws an explicitly-labelled flat track. That is an accepted limitation, not a defect — the alternative (retaining audio longer to enable backfill) would weaken a retention promise to improve a navigation aid. |
| **Archived videos (Issue 446)** | `videos.archived_at` soft-delete marker; the row, its clips, and `clip_feedback` are retained | Until restored, permanently erased, or account deletion | Archive is a LIBRARY state, not an erasure: stored media (source, extracted audio, every clip render) is purged at archive time by per-URI enumeration, and `archived_at IS NULL` removes the row from every surface — list, catalog, counts, Dashboard/Profile aggregates, the GDPR export's video list, and the backfill sweeps. Clip + feedback rows are retained as the preference model's training labels and still appear in the GDPR export (Art. 15 honesty about what is held). The retention-purge beat task deliberately still matches archived rows so an Object-Lock-refused delete is retried. Permanent paths: `POST /videos/{id}/erase` and `DELETE /auth/me`. |
...
| **Clip edit documents (Issue 391)** | `clip_edit_documents`: one row per edited clip — `doc` (JSONB: schema `version`, a `cuts` list of `{id, start_s, end_s}` in clip-relative seconds, `last_applied_at`) and an integer `revision` | Until account deletion (CASCADE via BOTH the `creator_id` and `clip_id` FKs), or until the clip itself is deleted | **Creator-authored editorial decisions** — which spans of their own clip to remove. No YouTube-origin data, no PII, no transcript text: only ids and float timestamps. It replaces `localStorage['clip:{clipId}:cuts']`, so the change moves this data class **from the creator's browser onto our servers**, which is what brings it in scope here at all — the justification is that the creator's work was previously destroyed by clearing site data or switching machines, with no warning. RLS `tenant_isolation` ENABLE+FORCE with the hardened `NULLIF(...)` GUC form (migration 0052). Included in the GDPR Art. 20 export (`_collect_creator_export`), scoped by `creator_id` directly. **Deliberately stores no derived state:** the transcript word span (`indices`) is recomputed on load rather than persisted, so this table never holds a stale copy of transcript-derived content. |

### Erasure vs. backups (Issue 254)

The right-to-erasure stance, stated honestly:

- **Live systems: immediate.** `DELETE /auth/me` revokes the OAuth grant, purges R2 media by
  per-URI enumeration over every write-site key pattern (`worker/erasure.py` — all videos
  including archived, every clip render variant, posters, peaks, extracted audio, recap
  renders, GDPR export bundles) plus creator-scoped prefix sweeps as belt-and-braces, purges
  telemetry, and cascade-deletes every DB row at request time.
...
- **R2 media has no versioning (DEC 2026-06-27, Issue 258):** the enumerated per-URI deletes
  plus the `source/{creator_id}/`, `posters/{creator_id}/`, `peaks/{creator_id}/`, and
  `exports/{creator_id}/` prefix sweeps remove the only copy — media erasure is immediate
  with no backup tail (DB dumps carry no media bytes). Renders live at non-creator-scoped
  `clips/{clip_id}.mp4` keys and are covered by the enumeration, which the Issues 446/471
  tests gate against every write site.
