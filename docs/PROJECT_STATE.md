...
**Active issue**: Issue 78 — re-implementing the net-new pieces salvaged from closed PR #6, one small test-gated PR at a time. 78a (scorer cache) ✅ done; next up: clip-scorer prompt caching (78b), mypy→0 (78c), improvement-brief 202/poll (78d), analytics retention purge (78e), PgBouncer harness (78f), legal/CORS delta (78g).
**Last completed**: Issue 78a — per-(creator, version) preference-scorer cache (per-worker LRU; `from_bytes` runs once per version, not per rerank). 430 passed; ruff 0 / mypy 30 / coverage ≥ floor.
**Blocked**: _(none)_
