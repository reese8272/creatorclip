...
| 326 | Beta observability — Grafana Cloud (logs+metrics+traces) + Sentry + OTel | OBS (W0 / L08_OBSERVABILITY) | 🟡 Code-complete, external-verify pending (2026-06-27) | OTel/Sentry code committed `e837979` (no-op until env set; 42/42 obs tests green). Activation wired to the **VM** (not Render — Render DB is empty): `deploy.yml` guarded secret-sync now syncs `SENTRY_DSN`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_EXPORTER_OTLP_HEADERS` + stamps `IMAGE_SHA`. No compose change (all services `env_file: .env`). DECISIONS.md 2026-06-27 entry. **Open external gates:** create Sentry project + Grafana Cloud stack, set the 3 GitHub secrets, then verify a live HTTP→Celery trace in Tempo + exceptions in Sentry. Push held until creds ready. |
| 255 | Off-box escrow of TOKEN_ENCRYPTION_KEY / JWT_SECRET_KEY / .env | DR (W0 / Disaster Recovery & Infra) | 🟡 Docs/runbooks complete; external escrow pending (2026-06-27) | RUNBOOKS "Disaster Recovery" section (4 failure modes + restore drill) + re-escrow step in key rotation; SECRETS two-leg escrow doc; DECISIONS entry; `test_dr_docs.py` guard. **Open (yours):** copy the 3 secrets to 1Password + GCP Secret Manager. Prereq for #256 being usable. |
| 256 | Nightly encrypted Postgres backup → separate R2 bucket + tested restore | DR (W1 / Disaster Recovery & Infra) | 🟡 Code-complete; staging restore-drill pending (2026-06-27) | `scripts/backup_pg.sh` (streamed pg_dump→gzip→openssl AES-256→R2; DB creds container-side, passphrase via `-pass env:`, no client deletes), config + `.env.example`, `test_backup_pg.py` (8 secret-hygiene/fail-fast) + `test_backup_config.py` (3). **Open (yours):** create `creatorclip-backups` bucket + Object Lock (Compliance) + nightly cron, run the quarterly restore drill. |
| 257 | Pre-migration safety dump in the deploy pipeline | DR (W2 / Disaster Recovery & Infra) | 🟡 Code-complete; staging verify pending (2026-06-27) | Gated pre-migration dump in BOTH `deploy.yml` + `deploy.sh` (reuses `backup_pg.sh`, `predeploy/` prefix); skips-with-warning until #256 backups configured, hard-aborts once they are; `test_ci_config.py` pins dump-before-`alembic` ordering in both paths. |
| 258 | R2 durability hardening — Bucket Lock + lifecycle | DR (W0 / Disaster Recovery & Infra) | 🟡 Docs/decision complete; external R2 config pending (2026-06-27) | DECISIONS: R2 has no GA versioning → Object Lock **Compliance mode** (Governance is admin-overridable); COMPLIANCE + RUNBOOKS document clips/ lock + source/ lifecycle + backup-bucket lock, reconciled with right-to-erasure (#254). **Open (yours):** apply locks + lifecycle rules in the R2 dashboard. |
| 198 | Personalization efficacy harness — NDCG/MAP/Kendall (the moat) | EVAL (W0 / Scoring, Eval & Preference) | 🟡 Core complete & locally verified; staging run pending (2026-06-27) | `tests/eval/metrics.py` (pure NDCG@5/MAP@5/MRR/Kendall-τ/chrono-split/paired-bootstrap, 12 tests) + `tests/eval/efficacy.py` (random/generic-signal/DNA+preference rankings, 4 local tests incl. real fit→predict) + `scripts/eval_efficacy.py`. DECISIONS methodology entry (sourced). **Open (staging):** DB integration test + real-data pooled/per-creator run. Linchpin for #199/#200/#201/#202. |
| 200 | Recency-decay half-life calibration + parameterize | EVAL (W1 / Scoring, Eval & Preference) | 🟡 Parameterized & verified; sweep pending (2026-06-27) | `_LAMBDA` derived from `DECAY_HALF_LIFE_DAYS` config (default 30); `.env.example` + decay test read config (28 tests pass). **Open (staging):** {15,30,60,90} held-out NDCG@5 sweep + concept-pivot via the #198 harness; default moves only if it clears the incumbent CI. |
| 199 | Adversarial clip-quality scenarios + aggregate pass-rate | EVAL (W1 / Scoring, Eval & Preference) | ✅ Done (2026-06-27) | 8 adversarial geometry fixtures (each guards a named failure mode) + ranking-aware fixture (DNA-preferred ranks #1 via `rank_candidates`, no live Anthropic) + aggregate 100%-pass-rate gate + `SCENARIO_FLOOR` 6→14. Reconciled the "eval harness hardened" pre-launch gate across CLAUDE.md + PROJECT_STATE.md. clip_engine suite 51 passed; fully local. |
| 201 | `performed_well` baseline-unit fix (Shorts vs long-form) | EVAL (W1 / Scoring, Eval & Preference) | 🟡 Code-complete; staging measurement pending (2026-06-27) | Baseline is now the median of the creator's published **Shorts** (format-matched), not the full-video `VideoMetrics.views` median (the bug that flipped nearly every Short to `performed_well=False`); sparse case (`<3` Shorts) defers via pure helper `_shorts_baseline_median` (3 unit tests). Kept the recency-aware 3× outcome multiplier over hard dominance (DECISIONS). **Open (staging):** before/after label-bias measurement via #198 harness. |
| 202 | Continuous eval — impression/position logging + standing report | EVAL (W1 / Scoring, Eval & Preference) | 🟡 Impression log complete; standing report deferred (2026-06-27) | `ClipImpression` model + migration 0037 (RLS-gated, indexed) + best-effort write in `list_clips` (never breaks the read path); integration test (field capture + cross-tenant isolation); DECISIONS + COMPLIANCE updated (no PII/YouTube data, cascade-purged on erasure). **Open (staging + #265):** per-retrain pooled-NDCG@5 emission + regression ratchet. |
