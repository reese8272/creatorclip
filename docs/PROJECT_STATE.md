...
**Active issue**: _(none in flight)_ — Wave 2 closed (Issues 84 + 92).
**Last completed**: Wave 2 / Issue 92 — universal progress visibility. Extended the Issue-86 SSE primitive to upload chain, render, catalog sync, and improvement brief. Uses deterministic IDs (video_id, clip_id) as stream keys for upload + render; Celery task_id for catalog sync + improvement brief. Improvement brief now streams via the `task_id` kwarg pattern mirroring `dna/brief.py::generate_brief`. Frontend wired in onboarding + insights templates; backend `stream_url` returned by all 4 upload/render/catalog/brief endpoints. 8 new wiring tests in `tests/test_progress_emit_wiring.py`.
