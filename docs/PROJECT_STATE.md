...
**Active issue**: _(none in flight)_ — Wave 4 (compliance + scale prep) closed.
**Last completed**: Wave 4 — 3-fix compliance + scale prep batch. Closes the Wave-3-introduced SEV2 (`routers/videos.py` upload aset_owner fail-open), the longest-standing billing race (refund pack_id partial UNIQUE migration 0013), and the **single largest gap before Google OAuth app verification** (Issue 75b — YouTube analytics 30-day retention purge per ToS §III.E.4.b, verified via industry-standards-researcher). SEV1 stays at 0; SEV2 expected 33 → 30 on next /assess. CLAUDE.md pre-monetization item "YouTube data-retention/refresh fully compliant" ✅ ticked.
