...
**Active issue**: _(none in flight)_ — Wave 3 hotfix batch closed (3 SEV1s + 3 SEV2s).
**Last completed**: Wave 3 hotfix batch — addressed post-Wave-2 /assess regressions + the carry-forward Stripe SEV1. SEV1 trajectory: 4 → 2 → 1 → 3 → **0**. Closes 3 SEV1s (improvement-brief streaming tools drop, aset_owner ordering, sync Stripe in async checkout) + 3 SEV2s (OAuth callback aset_owner gap, upload-chain done-before-clips, catalog sync skip-video silent gap). All 6 fixes ship together: 543 passed / 1 skipped / 89 deselected; Layer 0 ruff 0 / mypy 0 / format clean.
