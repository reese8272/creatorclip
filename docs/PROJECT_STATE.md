...
**Active issue**: _(none in flight)_ — Wave 5 closed.
**Last completed**: Wave 5 — SEV1 hotfix + cross-tab task persistence + frontend visibility. Fixes the SEV1 from post-Wave-4 /assess (3 remaining `aset_owner` sites now have the fail-open wrap; SEV1 returns to 0). Adds `static/activeTasks.js` library + `static/activityPanel.js` floating widget so background work (DNA build, catalog sync, improvement brief) survives page-to-page navigation and is visible on every authenticated page. Existing onboarding + insights flows now register with the global panel via `window.activeTasks`.
