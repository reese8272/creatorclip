...
## 2026-07-29 — Ready-pass Wave 2 (hardening + cleanup tail)

Health-Redis client lifecycle (2026-07-29, w2/test-flakes): moved from the shared_resources registry to per-lifespan ownership — acquire before yield, aclose in the SAME lifespan's finally, stack-restore of the module global (LIFO TestClient nesting). Why: the registry's replace-on-re-register semantics leaked every earlier lifespan's client to GC; a loop-affinity guard (worker/progress.py precedent) was rejected because it still drops replaced clients to GC. Source: FastAPI lifespan events doc (acquire-before-yield/release-after-yield) + empirical full-suite runs. Prod impact: none (single lifespan per process); shared_resources remains for import-time singletons only (docstring updated). **Date:** 2026-07-29

Test-suite hygiene policy (2026-07-29, w2/test-flakes): (1) tests must not bind loop-scoped async Redis clients to ephemeral event loops — TestClients whose requests reach Redis-touching code paths must be context-managed (bare TestClient = one fresh loop PER REQUEST, verified empirically on the installed starlette), or the Redis touchpoint must be patched (aset_owner/aemit convention); (2) tests that lazily create a loop-bound module singleton must close it on the creating loop in teardown (async autouse fixture, test_progress/test_progress_emit_wiring pattern); (3) pytest_sessionfinish disarms (_writer/_reader=None) any leftover dead-loop singletons — graceful aclose is impossible once the owning loops are gone, sockets are closed by transport __del__/process exit, and mid-run leaks still surface as PytestUnraisableExceptionWarning so real regressions are not masked. **Date:** 2026-07-29

2026-07-29 — Advisory-lock leak fix: guarded-invalidate over xact locks. The OFF_COURSE 2026-06-30 fragility (rollback failure in _rollback_then_unlock leaks the session-level lock onto a pooled backend) is fixed by wrapping rollback+unlock in try/except and calling AsyncSession.invalidate() on failure, NOT by switching to pg_advisory_xact_lock and NOT by unlocking from a fresh connection. Why: per postgresql.org functions-admin, pg_advisory_unlock from another backend returns false and releases nothing (session locks are per-backend), so a fresh-connection unlock is impossible; per explicit-locking, xact locks auto-release at COMMIT, which would forfeit whole-run mutual exclusion for the sweeps that commit per creator mid-run (_poll_clip_outcomes_async, _refresh_youtube_analytics_async). invalidate() discards the connection instead of pooling it, and PostgreSQL frees session-level advisory locks at session end even on ungraceful disconnect. One helper change covers all 8 pg_try_advisory_lock sites; skips are now observable via log_event(beat_lock_skip) + BEAT_LOCK_SKIPS_TOTAL{task} (per-creator key suffixes stripped from the label for cardinality). Evidence: postgresql.org/docs/current/explicit-locking.html, functions-admin.html; SQLAlchemy Session.invalidate docs. **Date:** 2026-07-29

2026-07-29 — chat.runner.run_chat_turn signature changed from (…, session: AsyncSession) to (…, session_factory: Callable[[], AbstractAsyncContextManager[AsyncSession]]) as part of closing the last three open-session-across-LLM holds (Issue 82b posture). The agentic loop needs DB access mid-turn (tool execution, usage-ledger write), so the plain read→LLM→write split does not apply; instead a short-lived tenant session is opened per tool call and for the ledger write (which now commits there), with shipped precedent in _generate_clips_async's ledger_session_factory / clip_engine.scoring. Caller passes functools.partial(db.tenant_session, cid) so every session is RLS-stamped per transaction. _build_dna_async deliberately remains un-split: its session + pg_advisory_xact_lock held across the LLM serializes concurrent builds (Issue 76 double-spend race). **Date:** 2026-07-29

2026-07-29 — clip_engine/scoring.py _transcript_context._gather: chose MIDPOINT assignment with half-open [start_min, end_max) bounds over captions.py-style pure-overlap selection when fixing the segment-straddling context gap. Why: pure overlap would place a straddling segment in TWO adjacent sections, duplicating tokens in the LLM scoring prompt; midpoint keeps [BEFORE]/[CLIP]/[AFTER] a strict partition so every segment lands in exactly one section. Matches the 2026-07-20 assessment's suggested fix (docs/assessment/modules/clip_engine.md). Side effect (intended fix direction): at the outer context edges, a segment straddling before_start/after_end whose midpoint falls inside now contributes where it was previously excluded — LLM prompt content only; no geometry/eval impact (eval harness is geometry-only via extract_candidates). Evidence: 115 focused tests green incl. all four pre-existing _transcript_context fixtures unchanged + new straddler regression test; branch w2/clip-engine commit f6839bd. **Date:** 2026-07-29

2026-07-29 — SPA fonts self-hosted via Fontsource VARIABLE packages (@fontsource-variable/geist, geist-mono, inter @5.3.0), replacing the fonts.googleapis.com @import in frontend/src/index.css. Why: remote Google Fonts transmits every visitor's IP to Google — LG München 3 O 17493/20 held this violates GDPR Art. 6(1) and named self-hosting as the remedy. Variable (not static) packages are load-bearing: the type scale uses weight 650 (--text-h1--font-weight), which only a continuous wght axis serves. Font tokens now lead with the Fontsource 'X Variable' family names, existing fallbacks kept. Verified: npm run build output contains zero fonts.googleapis/gstatic references; woff2 hashed under dist/assets with the /app/ base. Visual QA of h1 (650) + mono surfaces still recommended on next deploy. **Date:** 2026-07-29

2026-07-29 — Server-influenced URL sinks in the SPA now pass through frontend/src/lib/safeUrl.ts: URL-parser canonicalization + http(s)-only ALLOWLIST per OWASP XSS Prevention Cheat Sheet (relative URLs resolve against origin and pass; javascript:/data:/vbscript:/s3:/file:/unparseable return undefined so React drops the attribute). Deliberately NOT host-pinned to same-origin — that would break the fixed youtu.be link and future presigned R2 redirect targets. Applied at DashboardBanners next_action_url (with /app/onboarding fallback) and the dormant Timeline waveformImageUrl prop; PublishPanel youtube_video_id uses encodeURIComponent (ID into fixed prefix, not a URL). **Date:** 2026-07-29

2026-07-29 — Cleaned-preview <video> sinks (CleanedPreviewConfirm, TranscriptEditor, Editor) no longer bind the raw cleaned_render_uri; they use the existing authed GET /clips/{id}/download?variant=cleaned&disposition=inline (routers/clips.py). Why: cleaned_render_uri is an opaque storage URI — s3://bucket/key in prod R2 (worker/storage.py) — so the raw binding was already unplayable in production, and a sanitizer alone would have silently blanked it. The polled URI remains purely the readiness signal. Needs a live smoke after Deploy 2 (302-presigned redirect path). **Date:** 2026-07-29

2026-07-29 — notify/copy.py trial_ending/balance_low bodies now bind settings.APP_BASE_URL at import time (f-strings), mirroring the app_url Jinja global precedent in notify/mailer.py:51. Verified before building (brief contract correction): the host-less '/pricing' was reachable by NO runtime path — in-app notifications use worker/tasks.py _build_inapp_notification's own copy map and emails render Jinja templates with {{ app_url }}/pricing; notify.copy.COPY is imported only by tests. This is future-proofing of the canonical copy module; zero user-visible strings changed today. Note: notify.copy now has an import-time dependency on config.settings (consistent with mailer.py). **Date:** 2026-07-29

2026-07-29 — Funnel/lifecycle telemetry scheduling (ready-pass W2, routers-DRY lane): the 8 router-side asyncio.ensure_future(record_event(...)) fire-and-forget sites (auth oauth_started/oauth_completed; creators data_gate_evaluated/catalog_sync_started/dna_build_started/dna_confirmed/identity_saved; review clip_kept) now flow through the Issue-352 bounded strong-ref scheduler event_log.record_event_nowait instead. Why: per the official asyncio docs (docs.python.org/3/library/asyncio-task.html, 'Save a reference to the result of this function...'), the loop holds only weak refs to tasks, so an unreferenced telemetry task could be garbage-collected mid-execution and the funnel row silently lost. Behavior delta: with the DB sink enabled and >20 writes already pending, new events are DROPPED (Issue-352 bounded-backlog posture) where ensure_future previously queued unboundedly — deliberate alignment with the telemetry-is-best-effort rule. Evidence: python.org asyncio task-reference warning (fetched in the W2 CHECK brief); regression test tests/test_data_gate_delta.py::test_data_gate_event_task_strongly_referenced. **Date:** 2026-07-29

2026-07-29 — Issue 272 visual-baseline procedure: baselines are (re)generated ONLY via `gh workflow run ci.yml -f update_snapshots=true` (new boolean workflow_dispatch input) — the ubuntu-latest `visual` job appends --update-snapshots and uploads frontend/e2e/__snapshots__/ as artifact `visual-baselines-<sha>`; they are then committed in a dedicated PR that simultaneously removes `continue-on-error: true` so the PR's own CI validates the gating flip. Trigger: smoke.spec.ts documented a fictional `UPDATE_SNAPSHOTS=true` CI mechanism that never existed in ci.yml; ruled out local/WSL2 generation (font anti-aliasing differs from the CI runner — Playwright docs playwright.dev/docs/test-snapshots) and renaming failure-artifact actuals (fragile). Evidence: ready-pass W2 brief-e2e.json, ci.yml visual job, lane w2/e2e commit 1a2d743. **Date:** 2026-07-29

2026-07-29 — Billing audit (w2/billing-audit): three money-path fixes. (1) `billing.ledger.record_llm_usage` gained keyword-only `cache_write_multiplier` threaded to `_estimate_cost_usd`; the five `knowledge.util.dna_system_block` ttl:"1h" features (titles, thumbnail concepts, clip_titles, clip_captions, clip_explain) flag `cache_1h` in their usage dict via the new `has_1h_cache_marker` helper, and their five billing call sites (routers/clips.py ×3, worker/tasks.py ×2) pass `2.0` ONLY when the marker actually attached — closing the 1.25×-vs-2× 1-hour cache-write under-bill left by the 2026-06-24 entry (which fixed scoring.py but not these five). Simplification worth logging: the flag reads the BUILT system blocks (exactly what was sent to the API, not a re-measured floor), and passing 2.0 whenever flagged is safe even if the API declines to cache — `cache_creation_tokens` is then 0, making the multiplier moot. (2) `chat/intake.py run_intake_turn` (POST /creators/me/identity/chat) and (3) `knowledge/thumbnails.py analyze_thumbnail_patterns` (multimodal vision) were entirely unbilled (0× and invisible to the Issue-290 spend guard); both now bill through `record_llm_usage` at Sonnet rates incl. cache tiers — the vision call returns `(patterns, usage)` and bills inside BOTH compute paths (worker task + router single-flight) so Redis-cache hits and single-flight waiters bill nothing. Guard: `tests/test_usage_coverage.py` gained a repo-wide AST sweep mapping every `*.messages.create/stream` / stream-helper call site to concrete billing evidence, so the next unbilled LLM site fails CI. Pricing evidence: official prompt-caching docs (0.1× read / 1.25× 5-min write / 2× 1h write; `usage.input_tokens` = uncached remainder only), corroborated by the claude-api skill; full call-site classification in /tmp CHECK brief `brief-billing-audit.json`. **Date:** 2026-07-29

**Billing (approved money path, w2/billing-audit 452a700+f1bee23).** Cached-token audit
verified billing/ledger.py + all previously-suspected callers were ALREADY cache-correct;
the real defects were two entirely-unbilled sites (chat/intake.py `run_intake_turn`;
knowledge/thumbnails.py `analyze_thumbnail_patterns`, both compute paths) and a 1h-TTL
cache-WRITE under-multiplier (1.25× vs the official 2×) on the five `dna_system_block`
features. `record_llm_usage` gains keyword-only `cache_write_multiplier`; call sites pass
`2.0 if usage.get("cache_1h") else None` — applied ONLY when the floor-gated ttl:"1h"
marker actually attached (below-floor calls keep the 1.25× default; when the API declines
to cache, cache_creation_tokens=0 makes the multiplier moot). Pricing evidence:
platform.claude.com prompt-caching docs (0.1× read / 1.25× 5-min write / 2× 1h write).
Net: creators are charged for calls that previously leaked (~$0.055/cold thumbnail
analysis, ≤1/day/creator; intake turns at standard Sonnet rates; +$0.002–0.007 per cold
1h write) and newly-billed usage counts against the Issue-290 spend guard — approved by
the owner 2026-07-29. Structural guard: repo-wide AST sweep in tests/test_usage_coverage.py
fails CI on any unbilled `*.messages.create/stream` site. **Date:** 2026-07-29

2026-07-30 — Standalone Review & Editor landings (Issue 369, supersedes the Issue-304 Editor empty state): /review and /editor with no video_id now render a shared picker + upload-in-place landing (frontend/src/components/landing/) instead of dead-end messages pointing at the Dashboard/Review. Owner direction: each page must work as a tool in its own right (full vision filed as Lane L23, Issues 370–373). Three deliberate calls: (1) clip **generation stays an explicit click** — it spends creator minutes + LLM budget, so the landing never auto-chains it after ingest (billing consent), unlike the otherwise-seamless upload→progress→tool flow; (2) **no per-row StageStepper** in the picker — the server caps SSE at 3 concurrent streams per creator (progress.py MAX_CONCURRENT_SSE_PER_CREATOR), so only the single InlineUploadFlow card opens a stream and picker rows ride the shared 5s videos poll (extracted to lib/videosPoll.ts); (3) the landing's GenerateClipsButton **consumes the generate response body** (the Dashboard fires-and-invalidates) so clips>0 advances in place and 402/429/503 server-authored messages render verbatim — generate is idempotent (routers/clips.py Issue-61 guard), making the retry affordance token-safe. **Date:** 2026-07-30

2026-07-30 — Lane L23 batch build (Issues 372→373→370→371, /issue-workflow batch mode approved by owner). Per-issue CHECK evidence + the flagged design calls:
**372 (source streaming + transcript):** GET /videos/{id}/stream mirrors the clip-download pattern — 302 presigned R2 in prod, FileResponse in dev; dev Range/206 support verified against https://www.starlette.io/responses/ ("File responses also supports HTTP range requests… 206 Partial Content"); the prod redirect path is production-proven by existing clip playback. GET /videos/{id}/transcript is SEGMENT-granular (no nested word arrays — a 2h video's word payload is megabytes; search + click-to-seek need segment bounds; `include_words` can be added later), returns 200 empty_initial envelope (not 404) when untranscribed, and runs at 120/min (deviation from clip-transcript's 60/hour — editor panel refetches on video switch).
**373 (creator clips + export):** (1) Export scope — full source-edit export has NO pipeline support (render cuts one window; no concat/EDL renderer); the Export panel ships as an honest per-clip artifact list (rendered clips + real download links + aspect) with the stub REMOVED and one line saying source-edit export isn't available. Owner-approved reinterpretation of the AC. (2) Creator-clip bounds ≥2s/≤600s — deviates from the engine's 30s minimum (candidates.py MIN_CLIP_S) and the 5s cuts precedent: creator freedom, floored only against mis-drags and capped so one render stays bounded (worker timeout scales 4× duration). (3) Provenance = signals_jsonb.origin="creator" (no migration; JSONB precedent) — revisit if provenance ever needs indexing. (4) Soft dedupe (exact-range SELECT → 200 with existing row) instead of a fragile float-tuple UNIQUE constraint. (5) Creator clips EXCLUDED from ClipImpression logging — their NULL rank logged as 0 would poison the IPS/counterfactual top slot (Issue 202 data integrity). (6) Auto-render on create (rendering charges no extra minutes — paid at ingest) with best-effort enqueue: 201 even on broker failure; the manual render button is the recovery path. (7) UI: never a fake fit tier — "Your selection — not engine-scored" framing in ClipPlayer/WhyThisClip/LongFormEditor.
**370 (video-level style review):** video_feedback mirrors the ClipFeedback shape (valence → tags → note) with a NEW 2-value video_sentiment_enum (skip/trim/format are clip mechanics); tags capped ≤10×64 chars (they feed an LLM prompt — the clip endpoint predates the distiller and has no cap); POST REQUIRES at least one tag or note (bare valence teaches the distiller nothing); no LLM in-path → no flag/budget deps on the route.
**371 (style distillation):** (1) Injection is a THIRD system block AFTER the cached DNA block in clip scoring — never concatenated into brief_text: prompt caching is a byte-prefix match (https://platform.claude.com/docs/en/build-with-claude/prompt-caching.md via /claude-api, re-verified 2026-07-30 — "content after the last cache_control breakpoint is free to change"), so re-distillation would otherwise invalidate the creator's 1h cache each time, perturb the 1024-token floor gate, and desync the 2× cache-write billing coupling (scoring.py). CI pin: test_style_distill.py asserts the DNA block is byte-identical with/without notes. (2) Separate distill_style_prefs task, NOT an extension of retrain_preference — the retrain debounce early-returns on tag-only feedback and never sees video reviews, and the LLM call needs kill-switch + spend-guard gating the numeric fit doesn't. (3) Storage = single-row creator_style_notes table (creator_style precedent) — CreatorDna rejected (versioned confirmed/draft lifecycle; writing to a confirmed row mutates history), PreferenceModel rejected (pruned to 5 versions — the pruner would delete the notes). (4) Model = new ANTHROPIC_MODEL_STYLE_DISTILL="claude-haiku-4-5" registry key (Issue 318 compliance; /claude-api: $1/$5 per MTok, the hooks/chapters/performer tier). No cache marker (sub-floor static prefix, rare debounced calls — Issue 315 rule). (5) Distilled output is treated as UNTRUSTED on DNA rebuild — user-turn wrap_untrusted("learned_style_preferences") per the Issue-224 stated_identity precedent — and surfaced VERBATIM on the DNA card (it is exactly what scoring injects; honesty). (6) Usage-coverage AST registries extended (preference/style_distill.py + _distill_style_prefs_async) — the CI sweep now fails on any future unbilled path here. **Date:** 2026-07-30

2026-07-30 — **Positioning: lead with the outcome loop, not clip generation (Lane L24 filed, Issues 374–383).** Owner-requested product/market review. Three dated external findings, verified live 2026-07-30, change the strategic ground under the roadmap: (1) YouTube is rolling **Video Clips into Shorts plus auto-suggestions for the most "clippable" moments later in 2026** (tubefilter.com/2026/04/17/youtube-clipping-tool-viewer-clips-shorts/) — the "we find your best moments" layer becomes free and native, so it cannot carry the product; (2) YouTube's **16 July 2026** rename of "repetitious content" → **"inauthentic content"** makes mass-produced/templated output non-monetizable on a three-strike ladder, with clips monetizable only with "significant original value" (tubefilter.com/2026/07/13/…, techcrunch.com/2026/07/20/…, vidiq.com/blog/post/youtube-reused-content-policy-guide/) — this is an extinction-level risk for template-based clippers and a tailwind for a per-creator system, and it converts our honesty posture from overhead into positioning; (3) OpusClip's own 2026 strategy post concedes the bottleneck moved to "the decision of what deserves cutting" and that "measurement is where repurposing setups break down" (opus.pro/blog/short-form-video-strategy-2026) — the category leader publicly naming our north star as unsolved. A fresh sweep of the 2026 comparison field (Ssemble, Choppity, Vizard, quso) confirms **style-learning is still unclaimed** — brand kits and templates only — so the COMPETITIVE_RESEARCH.md:104 thesis holds 6 weeks on (dated re-confirmation, not an assumption). **Decision:** reposition from "an AI clipper that also knows your channel" to "the system that tells you what to cut, proves what worked, and keeps you monetizable." The load-bearing consequence is that the **already-built** outcome loop — `poll_clip_outcomes` → `ClipOutcome.performed_well = views >= channel_median` (worker/tasks.py:1055) → 3× training weight (preference/decay.py:8) → fed by #197's publish-time row creation — is the product's uncopyable asset (it requires per-creator Analytics OAuth, which no competitor has) and is currently surfaced to creators **nowhere**; Issue 374 makes it visible, and it needs no new data collection. Three prior decisions are explicitly touched and NOT silently reversed: the Phase-3 parking-lot item "no-auth demo mode" is proposed for promotion under #376 with the required fresh-approval entry; the #209/#125/#152 per-input-minute pricing lock is re-opened for *evaluation only* under #380 on three named new facts (one-shot vs recurring free minutes, metering-vs-training-data tension, and #377 reducing clips per input minute), with the MinuteDeduction ledger untouched either way; and #132's chat-spike BLOCK stands — #381 is a distinct live-capture path (`liveChatMessages.list` on the creator's own broadcast) that does not touch the scraper prohibition, gated on a quota verdict that may sink it. Issue 382 is the scope call that funds this (99 done / 614 open / 1 user on the OAuth cap). **Date:** 2026-07-30

2026-07-30 (later) — **Issue 383 correction pass: three claims in the entry immediately above did NOT survive primary-source verification, and Issue 382's premise was stale.** The 2026-07-30 positioning entry was written from secondary coverage; re-fetching the primary sources this session refutes three of its specific factual claims. The strategic direction (lead with the outcome loop) stands — the *evidence* for it needed repair, and two of the three bad claims were headed for creator-facing copy via #375/#378, where they would have breached the honesty constraint far more visibly than any clip score. **(1) There is no "three-strike ladder."** Neither support.google.com/youtube/answer/1311392 nor techcrunch.com/2026/07/20/… describes a warning → 90-day suspension → permanent-removal ladder for inauthentic content; YouTube's own page describes a range from limiting ad earnings to suspending or terminating monetization, with no numbered strikes. **#375 must describe consequences qualitatively and link YouTube's page rather than restating a mechanism we invented.** **(2) The rename was 15 July 2025, not 16 July 2026.** The 2026-07-16 event was a *clarification* splitting the existing policy into three named categories (generic/template-based; unsatisfying/off-putting; AI personas on sensitive topics). The policy is therefore ~13 months old, so "the single biggest strategic shift since this doc was written" and any "new policy" urgency framing in #375/#376 is wrong. The favourable substance survives: templated mass-production is explicitly non-monetizable and clips are monetizable with commentary/critical review/added narrative. **(3) The two OpusClip quotes do not exist in the cited source.** "the real bottleneck shifted to planning — the decision of what deserves cutting" and "measurement is where repurposing setups break down" appear in neither opus.pro/blog/short-form-video-strategy-2026 nor the adjacent creator-economy-2026-ai-video-repurposing-attention-war post (both fetched 2026-07-30). Opus's actual wording runs the other way — measurement framed as routine iteration, and the named bottleneck is "the gap between content creation and content distribution." **These quotes must never be published**; attributing invented statements to a named competitor is legal and reputational exposure, and #378 is by definition a trust surface. The finding survives via checkable substitutes: independent coverage notes the category "underexplains how to measure incremental performance after repurposing and how to decide which source assets deserve another pass" (superx.so/blog/content-repurposing-tools), and competitor reviews list "no cross-platform analytics" among Opus's concrete limits (playcut.ai, choppity.com). **Also corrected — the YouTube-native threat is real but narrower than stated:** Video Clips in Studio is *16:9 only and cannot generate Shorts*, its AI suggestions are *podcast-playlists only, English only, 10 countries* (support.google.com/youtube/answer/15824265, fetched 2026-07-30); the Shorts integration + auto-suggestions are *announced, not shipped* ("Later this year…", tubefilter 2026-04-17). Separately, the L24 filing conflated two opposite changes: the *viewer-facing* Clips feature is being **discontinued** (replaced by mobile timestamp sharing), which is a removal, not the expansion. Honest framing: the clip-finding layer is being commoditized *on a known trajectory*, not *already free and native*. **CONFIRMED unchanged:** style-learning is still unclaimed across the 2026 field (Reap, OpusClip, Vizard, VEED, Riverside, Submagic, ClipSpeed, Choppity, quso) — differentiation is editor quality, captions, reframing, languages, price, and now agent/MCP integration. **New competitor tiers added:** platform-native (YouTube itself) and agent-native (Reap ships an MCP server for Claude/ChatGPT/Codex). Evidence and per-claim links in docs/COMPETITIVE_RESEARCH.md § "2026-07-30 refresh — corrections". **Date:** 2026-07-30

2026-07-30 (later) — **Issue 382 `[DEC]` scope freeze: nothing meaningful left to park — fund all of L24; park only #363.** The issue was filed to "deprioritize the breadth cluster to fund the moat," but its premise did not survive checking. **Every item it proposed parking has already shipped:** #322 (per-clip titles) and #323 (thumbnail concepts) are DONE 2026-06-26; #324/#325 (agentic chat) are DONE 2026-06-26; L23 "advanced effects" (music/ducking, transitions, speed ramping, b-roll) were never filed as issues and are already explicitly parked in the L23 lane preamble (docs/issues.md:6278). The only live breadth candidate is **#363 (Caption TEXT editing)** — Size L, `Verify: render-env`, and already deliberately descoped from the ready pass by owner decision 2026-07-29. **Decision: park #363; fund #374–#380 + #383 and the hygiene cluster.** Un-park criteria for #363, per the parking-lot rule: a beta creator asks for caption-text correction unprompted, OR the #374 outcome data shows caption wording correlating with `performed_well`. **Reconciliation — the "99 done / 614 open" figure driving the L24 framing was a miscount** and is corrected everywhere it appears. 614 counted **acceptance-criteria checkboxes across done *and* open issues alike**, not open issues. Actual: 205 `### Issue N:` headings, 155 DONE/CLOSED/PARKED, **50 genuinely open** — of which ~25 are external/operator-gated (Google OAuth review, GKE, Cloudflare, DPAs), 5 are DESCOPED-BETA under the 2026-06-26 v1 lock (#58/#259/#261/#262/#287), 6 are code-complete pending external verification (#198/#200/#201/#228/#256/#257), and 2 are owner `[DEC]`s. **The backlog is not sprawling into breadth — it is already the moat lane plus hygiene**, which is why the scope freeze reduces to a single park. Consequence for #380: it is narrowed to **free-trial structure only** — live pricing re-verification (2026-07-30) shows AutoClip's per-minute *rate* beats OpusClip at every tier (9.0 ¢/min Starter vs ~10.0 ¢/min; 4.0 ¢/min Stream ≈ 2.5× cheaper) and per-input-minute is confirmed category-standard, so two of the three L24 pricing criticisms are unsupported; the verified gap is 60 free minutes **once** (config.py:588) vs 60/month recurring at both Opus and Vizard — a trial-design question needing no `MinuteDeduction` change. **Date:** 2026-07-30
2026-07-30 — Issue 365: static legal-page fonts vendored (GDPR — LG München 3 O 17493/20 remedy). `static/_design-tokens.css`'s `@import url('https://fonts.googleapis.com/css2?...')` was missed by the 2026-07-29 SPA font self-hosting fix (same entry above) — it loads on tos.html/privacy.html/accessibility.html, exactly the pages an EU visitor reads before signing up, leaking their IP to Google on every load. Fix: two committed woff2 files under `static/fonts/` (`inter-latin-wght-variable.woff2`, `jetbrains-mono-latin-wght-variable.woff2`), replacing the @import with local `@font-face` + `font-display: swap`. Deviated from the SPA precedent on delivery mechanism only: NO Fontsource npm dependency — `/static` has no bundler, so files were downloaded directly (`curl` against `fonts.googleapis.com/css2` with a modern Chrome UA, latin-subset block only) and committed as plain assets; owner explicitly ruled out adding a frontend/package.json dependency for this. Chose VARIABLE fonts over per-weight static files after confirming via fontTools (`fvar` table introspection) that Google serves the exact same physical woff2 file across all requested static weights for the latin subset — i.e. these ARE variable fonts (Inter wght 100–900, JetBrains Mono wght 400–800) — so one file per family covers 400/500/600/700 (Inter) and 400/500/600 (JetBrains Mono) instead of seven static files; `font-weight: <min> <max>` ranges on `@font-face` (not discrete values) per the CSS Fonts Module Level 4 variable-font matching algorithm, since no `font-variation-settings` are set explicitly. Latin subset only (v1 English-only scope, docs/issues.md v1 scope #3) — the Google-served "latin" unicode-range block (U+0000-00FF plus common punctuation), not latin-ext/cyrillic/greek/vietnamese. OFL-1.1 license text committed per-family (`static/fonts/OFL-Inter.txt`, `static/fonts/OFL-JetBrainsMono.txt`, sourced from github.com/google/fonts) — a redistribution requirement, not optional. Regression guard: `tests/test_static.py::test_static_assets_do_not_reference_remote_font_hosts` (fails CI if any `static/*.css`/`*.html` references `fonts.googleapis`/`fonts.gstatic`) + `test_design_tokens_font_face_srcs_resolve_to_committed_files` (fails if an `@font-face` src doesn't resolve to a real file under `static/fonts/`); the pre-existing Issue-99 palette pin test was updated to assert the ABSENCE of the Google Fonts host instead of its presence. Evidence: LG München 3 O 17493/20 (self-hosting remedy, cited in the 2026-07-29 SPA entry); MDN font-display docs (swap = small block period + infinite swap period); fontTools fvar introspection run locally against both downloaded files. **Date:** 2026-07-30

2026-07-30 (W1) — **Issue 368: Layer-0 coverage measurement made deterministic; `coverage_line_rate` baseline moved 75.20 → 77.00 (a tightening).** Root cause reproduced, not assumed: `gate_coverage()` passed one `--cov` per source package, emitting **6 `<source>` roots**, and coverage.py relativizes each file against a matching root — so files under a root that is itself a package flattened to bare filenames and landed in package `"."`. Measured before the fix: packages were `{".", "preference", "youtube"}` with **`clip_engine` absent entirely**, so `_module_line_rate("clip_engine")` returned `None`, the gate logged "unknown", and its floor sat at an unenforceable 0.0. Fix: a single `--cov .` root (docs: coverage.py `[run] source`/`source_pkgs`/`relative_files` + `[xml] package_depth`, coverage.readthedocs.io/en/latest/config.html) — verified to yield one `<source>` and stable package names. `[tool.coverage.run] omit` in pyproject.toml already excludes `tests/`+`alembic/`, so the single root does not widen the measured set to test code. **Second, separate defect found while ratcheting:** the single-file fallback in `_module_line_rate` matched on filename SUFFIX, so the key `auth` matched BOTH root `auth.py` (100.0) and `routers/auth.py` (93.3) and the winner depended on XML iteration order — meaning the "auth 93.3" floor recorded 2026-07-29 was actually *routers/auth.py*, and the floor's meaning could change silently between runs. Resolution rule is now: exact filename match wins; the suffix fallback applies only when unambiguous; two-or-more suffix candidates return `None` ("unknown") rather than guessing, because a wrong rate silently mis-gates a load-bearing module. **Floors ratcheted** off the now-deterministic measurement with ~1.5pt margin: `clip_engine` 0.0 → **91.0** (measured 92.51), `preference` 0.0 → **88.0** (measured 89.64), `auth` 91.0 → **99.0** (now pinned to root auth.py, measured 100.0); crypto/limiter unchanged at 99.0. These two were never low-coverage — they were *unmeasured*, and had been reading 0.0 across every assessment run since the floors were introduced. **Baseline justification (CLAUDE.md Phase-4 requires this):** the global rate moved **UP** 75.20 → measured 77.16–77.22, because `--cov .` includes root modules the six-root invocation missed. Committed as **77.00**, deliberately ~0.16 BELOW the lowest observation: measured across three test orderings the global rate jitters (77.16 / 77.22 / 77.16) while per-module rates are stable, and the gate compares with only a 0.01 tolerance — pinning the exact measurement would have shipped a gate that flakes on test order. Regression tests in `tests/test_layer0_module_coverage.py` pin the exact-beats-suffix rule, the ambiguous→None rule, and a guard that no floor may sit at an unenforceable 0.0 again. **Date:** 2026-07-30

2026-07-30 (W3) — **Issue 246 reconciled to CODE-COMPLETE; lifecycle email stays DISABLED for the friend beta (owner decision).** The tracker listed #246 as an open `M` build, but verification found the whole feature already shipped: three template pairs under `notify/templates/`, the welcome trigger at `routers/auth.py:320`, the daily `worker.tasks.run_lifecycle_scan` beat task (`worker/tasks.py:954`, scheduled `worker/schedule.py:76`), the shared 48h frequency cap across welcome/nudge/re-engagement (`worker/tasks.py:3360-3444`), the `email_lifecycle` opt-out column, RFC 8058 one-click unsubscribe (`notify/mailer.py:171`), and 13 passing tests. **The only blocker is an operator value, and the code already fails safe:** `MAILING_ADDRESS` is `""` (`config.py:755`) and `config.py:752` makes `send_notification` skip every lifecycle email while it is unset. That is correct, not a defect — **CAN-SPAM requires a valid physical postal address on commercial (non-transactional) mail**, and these three are commercial-leaning, so they do not qualify for the statutory "transactional or relationship" exemption. Requirements confirmed 2026-07-30 (ftc.gov CAN-SPAM compliance guide via search; direct fetch 403s): an opt-out must be honored within **10 business days**, the opt-out mechanism must stay live **≥30 days** after send, and a valid physical postal address — street address, USPS-registered PO box, or registered CMRA mailbox — must appear. **Decision:** do NOT invent or placeholder an address, do NOT remove the beat entry, and do NOT ship lifecycle mail for the friend beta (it needs none). Tracked instead as an operator gate in `docs/GO_LIVE.md` § Compliance & Privacy, with the note that the address becomes public on every lifecycle footer. Residual after the address is set: the `staging` beat-scan verification over real product state, which this box cannot run. **Date:** 2026-07-30
2026-07-30 (W3) — **Issue 376(b) (no-auth demo mode) DESCOPED — reversible, owner decision.** Issue 376 shipped in two halves; only 376(a) (the public marketing landing page) is built this pass. 376(b) proposed a no-signup "paste a video, get 3 watermarked clips" funnel, promoting the Phase-3 parking-lot item "no-auth demo mode." That promotion required a fresh approval per the parking-lot rule, and it does not clear ToS review: the only demo source that is actually ToS-clean is a small set of pre-processed **house sample videos** (the Opus "try a sample: Vlog/Podcast/Sports" pattern), NOT an arbitrary third-party YouTube URL — `YTDLP_ENABLED=False` by default and `youtube/ingest.py:89` restricts ingestion to the creator's own authorized channel, so downloading and clipping someone else's public video for an anonymous demo would breach that own-content-only gate. A house-sample demo is real to build (rate-limiting, cost caps, and a watermark render path are all net-new), but it would demonstrate exactly the wrong layer for this lane's thesis: it showcases the commodity clip-cutting step (find-a-moment, cut-a-clip) that Issue 383's positioning research says is being commoditized and given away free by YouTube itself — not the outcome loop (Proof of Lift, Issue 374) that is this product's actual differentiator, because Proof of Lift requires a real creator's own Analytics history and has nothing to show for a stranger's sample video. Building 376(b) now would spend build budget reinforcing the weaker story. **Decision: ship 376(a) only.** `docs/issues.md` Issue 376 is updated to reflect this split. **Reversible** — un-park criteria: a house-sample demo is worth revisiting if either (a) friend-beta signup conversion off the 376(a) landing alone is measurably weak (Issues 26/28/282 give real data), or (b) a later lane wants a "before" picture to contrast against Proof of Lift (e.g. "here's an unpersonalized cut vs. here's what your own channel's data says") — that framing would sell the connect step honestly rather than showcase the commodity layer standalone. **Date:** 2026-07-30
2026-07-30 (W3) — **Issue 375: Originality guard built on Voyage content similarity, NOT structural fields, and threshold left explicitly unvalidated.** Two decisions, both driven by the #383 correction pass (see the two entries above) and by re-reading the issue's own acceptance criteria against its approach sketch, which were in tension with each other.

**(1) The gate is embedding cosine similarity over each clip's own transcript excerpt, not the structural fields (duration bucket, setup lead, source region, style_preset) the approach sketch lists first.** The issue's own acceptance criteria require "no false-alarm spam on creators with a legitimately consistent format" — but duration, setup lead, source region, and render style are exactly the things a creator sets DELIBERATELY for a consistent brand, which is the precise use case `docs/SOT.md`'s DNA feature serves. Gating on structural match alone would flag that creator for being consistent with themselves, failing the issue's own honesty bar. Resolution: `knowledge/originality.py::build_advisory` clusters clips by Voyage-embedding cosine similarity of the clip's OWN spoken content (sliced from `Transcript.segments_jsonb` via the new `knowledge/util.py::extract_transcript_range`, embedded via the new `dna/embeddings.py::embed_clip_excerpts` — reuses the existing Voyage singleton, no new client). Structural fields are computed and returned only as corroborating detail (`common_structural_features`) on an ALREADY-flagged cluster, never the trigger. Pinned by `tests/test_originality.py::test_consistent_format_alone_does_not_trigger_the_advisory` (five clips, identical structure, orthogonal embeddings → not flagged) and `test_a_meaningful_cluster_of_near_identical_content_is_flagged` (identical embeddings, one distinct → flagged).

**(2) `ORIGINALITY_SIMILARITY_THRESHOLD=0.92` and `ORIGINALITY_MIN_CLUSTER_SIZE=4` are UNVALIDATED and deliberately conservative — this could not be measured in this environment.** The issue explicitly warned against copying the commonly-cited ~0.85 near-duplicate cosine cutoff on authority, since some newer embedding models compress the useful range so far that ~0.45 is the equivalent cutoff for THOSE models — and this environment has no live `VOYAGE_API_KEY`, no Postgres/pgvector, and no real creator clip corpus to measure the actual same-channel pairwise-cosine distribution for voyage-3.5 (same-channel clips already share vocabulary/topic/speaker, which inflates baseline similarity versus a cross-channel comparison the public cutoffs were measured against). Per the issue's own fallback instruction ("if you cannot measure it, choose a deliberately CONSERVATIVE value and document that it is unvalidated"), the threshold was set HIGH (0.92 — fewer alarms) rather than guessed at either public number, and `ORIGINALITY_MIN_CLUSTER_SIZE=4` requires a real cluster (a single near-identical pair is treated as a possible coincidental callback, never flagged — `test_a_single_near_identical_pair_never_triggers_an_advisory`). Both live in `config.py`/`.env.example` with the unvalidated-recalibrate-before-scale warning inline, not buried here. **Must be recalibrated against real creator embeddings before this is trusted at scale** — tracked as a follow-up, not filed as a new issue yet.

**Copy, verbatim (load-bearing, `knowledge/originality.py::_build_message`):** *"N of your last W clips are near-identical in spoken content. [They also share the same X, Y.] YouTube's guidance on original content says mass-produced or repetitive output may affect monetization eligibility — see https://support.google.com/youtube/answer/1311392 (checked 2026-07-30). This is an advisory based on your own recent clips, not a certification of monetization eligibility or a statement from YouTube."* Contains no strike count, no suspension timeline, and no "new policy" framing — pinned by `tests/test_compliance_no_virality.py::test_originality_advisory_copy_has_no_strike_ladder_or_new_policy_language`, which scans the actual generated message text, not just static source.
...
2026-07-30 (W4) — **Issue 378 RE-SCOPED: publishing pooled cross-creator ranking metrics is prohibited by the YouTube API Services Developer Policies; we publish the synthetic eval-harness number instead.** The issue proposed publishing "the pooled micro-average across consenting creators" from `preference/efficacy.py` (NDCG@k on chronological holdouts with paired-bootstrap CIs) as a trust surface. Fetched the Developer Policies 2026-07-30; three clauses settle it, quoted verbatim: **III.E.2** — *"Do not aggregate API Data except that you may only aggregate API Data relating to YouTube channels that are under the same content owner as recognized by YouTube pursuant to content licensing agreement(s)… Such aggregated API Data must only be viewable by that content owner."*; **III.E.4.h** — *"API Clients must not (i) replace API Data with similar, independently calculated data, or (ii) access or use API Data to create new or derived data or metrics."*; **III.E.3.b** — *"API Clients must not display or allow access to Authorized Data to anyone other than the authorizing user or agents expressly approved by that user."* Our beta creators are **not** under a common content owner, so cross-creator aggregation is barred outright; and even *permitted* same-owner aggregation must stay viewable only to that owner, which independently rules out putting it on a public page. **Creator consent does not rescue this** — it is a Google-to-us term, not a data-subject permission, so it is not the creators' to grant. Note this does NOT constrain Issue 374 (Proof of Lift), which shows a creator their own data — expressly permitted by III.E.3.b. **What we publish instead:** the adversarial clip-geometry pass rate from `tests/eval/scenarios/` — **16 scenarios, all passing** — with methodology and explicit caveats. Those scenarios are hand-authored synthetic YAML timelines (energy spikes, retention peaks, silences, expected windows) containing **zero YouTube data**, so no policy clause applies. It is also a *stronger* trust claim than a pooled NDCG would have been: a reader can clone the repo and re-run it, whereas an aggregate over private creator data is unverifiable by the person being asked to trust it. The page states plainly what the number does not mean (synthetic fixtures; no performance prediction) and states the no-pooled-aggregate position with its ToS basis. **Drift guard:** `tests/test_eval_transparency.py` fails CI if the published count diverges from the real file count, if a named example scenario does not exist (fabricated evidence on a page whose argument is "this is checkable"), or if the ToS disclaimer is removed. Both guards were verified with negative controls, not just asserted. **Date:** 2026-07-30
---
...
2026-07-30 (W4) — **Issue 377: shortlist size fixed at N=3, not confidence-derived; generation/scoring volume unchanged (no LLM-cost saving made).** Two decisions, both driven by the constraint that the shortlist cut must be provably a PRESENTATION change, never an engine change (the eval harness geometry — `tests/test_clip_engine.py::SCENARIO_FLOOR` — is untouched by this issue and stays green byte-identically).

**(1) Fixed `SHORTLIST_SIZE=3` (`config.py`), not confidence-derived.** `clip.score` is a per-video Claude estimate with no cross-video calibration — there is no held-out data in this environment establishing what score threshold means "confident enough to be a top pick" versus "the rest of a mediocre batch," and CLAUDE.md's standing rule is to prefer a small fixed N over an unjustified score cutoff when calibration evidence doesn't exist. A fixed N is also directly anchored to the issue's own product framing ("these three, and here is the case for each" — `docs/issues.md` #377) rather than an invented number. Implemented as `clip_engine.ranking.is_shortlisted(rank, size=None)` — a pure predicate over the ALREADY-computed `rank` (never touches `score`/`rank`/ordering), defaulting to `settings.SHORTLIST_SIZE`. `rank is None` (a creator-made selection, Issue 373 — never engine-scored) is never shortlisted: there is no principle/reasoning to argue a case with, so a "top pick" badge on it would be dishonest. Wired onto `ClipOut.shortlisted` in `routers/clips.py::_clip_response`.

**(2) Generation/scoring volume is UNCHANGED — explicitly no LLM-cost saving was made, and none should be implied.** The issue floated this as a possible saving, but the guardrail that "show all candidates" must show the REAL full candidate set (not a subset dressed up as complete) requires every candidate to still be extracted and LLM-scored exactly as today (`clip_engine.candidates.extract_candidates` → `clip_engine.scoring.score_candidates`, `max_candidates=settings.CLIPS_PER_VIDEO_DEFAULT=8`, unchanged). Only the RANKED result is filtered for default display. Quantified: **0% reduction in LLM scoring calls per video** — the shortlist is a read-time cut in `_clip_response`, computed per response from the already-persisted `rank`, not a generation-time decision. A future version could explore scoring only the top-K signal-score candidates before the LLM pass (a real engine change, out of scope here and would need its own CHECK against `clip_engine/candidates.py`'s NMS/window logic).

**Frontend:** `frontend/src/pages/Review.tsx` filters the one-clip-at-a-time review queue to `clip.shortlisted` by default (WhyThisClip was already default-open/primary content since the Issue 306 redesign — no change needed there) and exposes a "Show all N candidates" toggle (`data-testid="shortlist-banner"`) that is the one-click, load-bearing escape hatch; falls back to the full set automatically when nothing is shortlisted (e.g., a video with only a creator-made selection). No new principle registered in `docs/CLIPPING_PRINCIPLES.md` — every shortlisted clip still cites its existing per-clip principle; the shortlist cut itself carries no separate "confidence principle" since it isn't a scoring signal, just a rank-based display cut.

**Verification:** `tests/test_shortlist.py` (10 tests: boundary/ties/fewer-than-N/zero-candidates/creator-selection cases for `is_shortlisted`, an explicit NDCG-invariance test against `preference/efficacy.py::ndcg_at_k` proving the cut cannot regress ranking quality because it never changes ranking, and router wiring). `tests/test_clip_engine.py -k eval_scenario` — 19 passed, `SCENARIO_FLOOR=15` unaffected. Full backend suite on this branch tip: 2446 passed, 70 skipped, 173 deselected (vs. 2436 passed / 70 skipped / 173 deselected measured on the same tip before this change — net +10, all new, zero regressions). Frontend: `npx vitest run` 334 passed (331 baseline + 3 new `Review.test.tsx` shortlist-mode cases), `npx tsc -b` clean. **Date:** 2026-07-30

2026-07-30 (W4 close) — **Layer-0 harness: two gates were measuring the wrong thing. Both fixed; `coverage_line_rate` baseline corrected 77.00 → 83.00. This SUPERSEDES the 77.00 figure set earlier today in the Issue-368 entry.** **(1) `pip_audit` audited the wrong Python interpreter.** `_have()`/`_run()` resolved tools by bare name through `PATH`, and the `pip-audit` there was `~/.local/bin/pip-audit` with shebang `#!/usr/bin/python3` — so the gate audited the **user's system Python (200 deps / 103 vulns)** instead of the project venv (**171 deps / 1 vuln**, that one being the already-logged pytest PYSEC-2026-1845, which `PIP_AUDIT_IGNORES` covers via its GHSA alias). The gate had been reporting ~96–103 phantom vulnerabilities unrelated to this codebase, and the 2026-07-29 session mis-diagnosed the identical symptom as "venv staleness" — that diagnosis was wrong and is retracted. Every Layer-0 tool resolved outside the venv (`ruff`, `mypy`, `bandit`, `pytest`, `pip-audit`); it only *mattered* for the environment-sensitive one, but the pattern was fragile for all of them. Fix: `_PYTHON_MODULE_TOOLS` + `_tool_cmd()` bind these to `sys.executable -m <module>`, with `_have()` probing by import using the same argv the gate will actually run, so availability and execution can never disagree about which tool is meant. Result: `pip_audit` 96 → **0**. **(2) The Issue-368 single-root switch silently widened the coverage denominator.** Moving to `--cov .` (correct, and necessary for stable package names) also swept in operational tooling that the previous multi-root invocation never measured — `scripts/llm_harness.py` (0%), `scripts/drills.py` (0%), `scripts/live_smoke.py` (40%), `scripts/doctor.py` (50%), and `run_layer0.py` itself (22%). These are run-by-hand utilities, not app code, and are untested by design; including them moved the goalposts on the global rate and made the gate fail at 76.39 for a reason that had nothing to do with the code under test. Fix: add `scripts/*` and `.claude/*` to `[tool.coverage.run] omit` in `pyproject.toml`, restoring the ORIGINAL measured scope (app packages) while keeping the package-name determinism the single root buys. **True app-code coverage is 83.5%**, higher than either prior baseline — the app was always better covered than the harness reported. Baseline set to **83.00** (~0.5pt margin; the global rate jitters ~0.06 across test orderings while per-module rates are stable). Layer 0 now fully green: ruff 0 · mypy 0 · coverage 83.57 · module floors all pass · bandit 0/0 · pip_audit 0. **Date:** 2026-07-30

2026-07-31 (W5) — **Issue 380 `[DEC]`: the free trial stays a ONE-TIME 60-minute, 7-day grant. No recurring free tier, no subscription, no code change. The "we are strictly worse at the top of the funnel" premise does not survive contact with what the competitor free tiers actually contain.** This entry is the decision Issue 380 exists to make, and it closes the last of the three premises #383 did not already retract.

**What #380 originally claimed, and what is left of it.** #383 (2026-07-30) live-re-verified competitor pricing and retracted two of #380's three premises: AutoClip's per-minute rate **beats OpusClip at every tier** (9.0 ¢/min Starter vs ~10.0; 4.0 ¢/min Stream ≈ 2.5× cheaper), and per-input-minute is **confirmed category-standard** (Opus, Vizard, Klap). #380 was then narrowed to a single question: our **60 minutes once** (`config.py:633` `FREE_TRIAL_MINUTES=60`, granted only on the `is_new` branch at `routers/auth.py:187`, expiring via `trial_ends_at = now + TRIAL_DURATION_DAYS`, `config.py:636` = 7) versus **60 min/month recurring** at both OpusClip and Vizard. That framing is also wrong, for a reason the earlier passes missed.

**The comparison was never apples-to-apples — the units differ.** From the verified pricing table in `docs/COMPETITIVE_RESEARCH.md:63`: **OpusClip Free is 60 min/mo *with watermark, 3-day storage, and no virality score*.** Vizard Free is 60 min/mo (`:65`); the field-wide pattern recorded at `:43` and `:80` is "freemium funnel with watermark on free tier", and `:80` states plainly that the watermark **is** the growth loop — "every posted short is an ad… removing the watermark is the #1 reason to upgrade." AutoClip's 60 trial minutes are **full-fidelity, un-watermarked, permanently stored output**. So the competitor grant is a *marketing surface that degrades the artifact*; ours is *the actual product*. Matching the number while ignoring the restrictions would not be reaching parity — it would be giving away, every month and forever, the thing they never give away at all.

**Cannibalization math, in our own pack units.** An unrestricted 60 min/month recurring grant is **720 free minutes/year — 3.6× the entire Starter pack** (200 min / $18.00, `billing/packs.py`), i.e. ~$65/yr of product handed over at Starter rate. A creator publishing one 60-minute upload a month would sit permanently inside the free tier and have no reason to ever convert. That is the textbook freemium cannibalization failure, and it lands hardest on exactly the long-form creator the Stream pack was built to win.

**Conversion evidence — the structural argument, and the decisive one.** 2026 benchmarks separate these models by a wide margin: opt-in (no-credit-card, time-boxed) trials convert **8.9–25.2%** depending on study, while freemium/recurring-free-tier converts **2–8% (median ~4.5%)**, with standard freemium products at 3–5%. Hard paywalls show a median Day-35 trial-to-paid of **10.7% vs 2.1% for freemium apps**. The market has moved accordingly: **57% of products now use free trials against 26% on traditional freemium**, a shift away from freemium specifically attributed to AI/infrastructure cost — the named 2026 failure mode is the "**Freemium Death Spiral**", where the cost of carrying free users outruns the revenue they convert into. Converting our time-boxed trial into a recurring free tier would therefore trade a structurally higher-converting mechanic for a structurally lower-converting one, in the year the category is abandoning it.

**COGS is explicitly NOT the reason — correcting Issue 380's own brief.** The brief implies recurring free minutes carry a prohibitive cost floor. They do not. `docs/research/findings/06_monetization_unit_economics.md` §2.3 puts a 60-minute upload at **~$0.32–0.44 total direct cost** (transcription $0.26 · scoring $0.02–0.06 · R2 storage ~$0.02 · render $0.02–0.10), so a recurring 60-min monthly grant would cost roughly **$0.35–0.45 per creator per month — under $45/month even at the full 100-user beta cap**. That is affordable. Anyone revisiting this decision must not lean on a cost argument that the unit economics do not support; the argument is conversion structure and artifact value, not spend.

**Margin floor re-checked (Issue 380 AC #3).** Unchanged, because the chosen option changes nothing: finding 06 §2.3 shows **~71–89% gross margin** across the 10-min / 60-min / 3-hr rows at Studio rates and higher at Starter, and states that margin "**never**" goes negative on a single honestly-deducted video at any pack price. It inverts only when outputs decouple from deducted minutes (re-render/API fan-out) — which the finding itself calls "**a quota problem, not a pricing problem**", and that guard already shipped: stacked per-creator rate limits (#228) plus the spend kill-switch (#290).

**A structural incoherence that would have to be resolved first anyway.** A recurring monthly grant contradicts the existing **time-boxed** trial: `trial_ends_at` is stamped once, in the same transaction as the grant (`routers/auth.py:194`, Issue 126, deliberately so the two states can never disagree). "Recurring" and "expires in 7 days" are not compatible semantics; option (b) is not a config edit but a new grant lifecycle plus a scheduled top-up job.

**Options scored, and why (d) is the real runner-up rather than (b).** (a) **status quo — CHOSEN**; (b) recurring free minutes — rejected on conversion structure and cannibalization above; (c) base subscription with included minutes on the existing ledger — no new evidence since #152 rejected flat subscription, and #383 removed the price-competitiveness motive that would have justified reopening it; (d) status quo **+ a watermarked free tier** — this is the *actually* standard-conformant version of what Opus and Vizard do, and is the option to revisit first. It is deferred, not dismissed, on two grounds: **no AutoClip-branded watermark render path exists** (the only watermark in the codebase is the creator's *own* brand-kit logo, `frontend/src/pages/Settings.tsx:203` — a creator-facing feature, not a free-tier stamp), and a viral-watermark growth loop is a **top-of-funnel acquisition mechanic aimed at a funnel that is currently capped at 100 invited users** (Google OAuth app: In production / External / 1-of-100). It buys nothing until signups open.

**What changed since #209 — stated explicitly, as CLAUDE.md requires for revisiting a locked decision.** #209 locked per-input-minute with a taper plus the Stream pack and ruled out a subscription. Of the three changes #380 cited as grounds to reopen: (1) the free-tier gap **is not the gap it appeared to be** — the competitor grants are watermarked and storage-limited (this entry); (2) the "metering makes creators ration the long content we want" tension is real but is addressed by the Stream pack's 4.0 ¢/min rather than by giving minutes away, and #383 showed we already undercut Opus at every tier; (3) "#377 returns fewer clips per input minute" is **factually wrong about #377** — the shortlist is a read-time display cut in `routers/clips.py::_clip_response` over the already-persisted `rank`; generation and scoring volume are unchanged (0% reduction in LLM scoring calls), and the full candidate set stays one click away. Nothing in #209's rationale is invalidated; it stands.

**Ledger untouched**, as #380 requires: `MinuteDeduction UNIQUE(video_id)` (#125) is not modified, and no option chosen or deferred here implicates it.

**Re-open trigger (explicit).** Revisit when **either**: (1) signups open beyond the 100-user OAuth cap — at which point option (d), a watermarked recurring free tier, becomes a real acquisition loop and should be costed against the §2.3 render line; or (2) beta conversion data shows trial-to-paid drop-off concentrated at the Starter boundary, which would indict pack sizing rather than trial shape. Absent one of those, this is churn.

**Source / evidence.** `docs/COMPETITIVE_RESEARCH.md:43,63,65,80` (competitor free-tier terms, live-verified by #383 on 2026-07-30); `docs/research/findings/06_monetization_unit_economics.md` §2.3 + §2's "quota problem, not a pricing problem" finding, and its own open question at :372 ("keep 60 min + 7 days, or move to an outcome-based 'first clip on us' grant") — now answered: keep 60 min + 7 days; 2026 trial-vs-freemium conversion benchmarks (ChartMogul SaaS Conversion Report; Artisan Strategies "State of Freemium 2026" and "SaaS Conversion Rate Benchmarks 2026"; Growth Unhinged 2026 free-to-paid conversion report), consulted 2026-07-31.

**Code changes: none.** `FREE_TRIAL_MINUTES=60` and `TRIAL_DURATION_DAYS=7` stand; `billing/packs.py` and `frontend/src/pages/Pricing.tsx` are untouched, so the known frontend/backend pack duplication (#209) is not aggravated and the no-virality disclaimer in all pricing copy is preserved unchanged. **Date:** 2026-07-31

2026-07-31 (W5) — **Issue 355: two structural calls on the first-run IA — the three "ask" surfaces are differentiated by SCOPE rather than merged, and the L24 insight panels get anchors rather than routes.** Both deviate from what the issue brief floated, so they are recorded here. Everything else in #355 (nav renames, blurbs, one primary CTA, empty-state prompts) is the brief executed as written.

**(1) The three ask surfaces are NOT merged. One nav entry was removed instead.** The brief suggested "consider merging Assistant+Analyze". Rejected: they are different jobs on different protocols. Assistant is channel-wide and conversational — `POST /api/chat/messages` then a multi-turn SSE stream with regenerate/stop (`frontend/src/pages/Chat.tsx`). Analysis is one-video and tool-shaped — `POST /creators/me/video-analysis`, a one-shot task stream (`components/analysis/AnalysisQuery.tsx`). Insights is channel-wide and *automatic*, answering without a question being asked. Merging the first two means one input box driving two protocols plus a merged transcript model — a redesign, which #355 explicitly is not ("Not a redesign… this is a naming + first-run pass"). **What was actually duplicated was the entry point, not the surface:** arriving at `/analysis` *from the nav* rendered only a URL field and a free-text question, because the four generators (`TitleOptimizer`, `HookAnalyzer`, `ChaptersPanel`, `ThumbnailConcepts`) are gated on `?video_id=` (`pages/Analysis.tsx:41-51`) — i.e. nav-Analyze was a strictly worse Assistant. So `Analyze` was dropped from `LINKS` while `/analysis` stays fully live, reached from where a video is already in hand: the `VideoTable` "Titles" row link, a new "Analyze this video →" in the `VideoClipsMap` header, and the new `components/ask/AskSurfaceTabs.tsx` row mounted on all three pages. That row delivers the brief's "clearer labels + a one-liner each" once instead of three times. Arriving at `/analysis` with no `video_id` now states that the four tools need a video, rather than silently hiding most of the page.

**(2) Proof of lift / Originality guard / Channel fingerprint stay PANELS with hash anchors — they did not become routes.** The obvious fix for "these are buried mid-scroll in `/insights` with no URL" is to give each its own route. Rejected, because it makes finding 2 (too many undifferentiated destinations) worse, not better: three more channel-level report URLs would each need their own `DisclaimerBand`, loading, and error chrome, plus either a nav slot or no discoverability at all. Their order on the page is also load-bearing — `pages/Insights.tsx` documents Proof of lift as deliberately first, being "the only falsifiable claim on this page", with the originality guard as an advisory over the same corpus. The real requirement was **addressability**, not routing: `static/landing.html` and `pages/Landing.tsx` both headline "Proof of lift: we measure what actually happened" with nowhere to click. Implemented as an optional `id` on `Panel` (`components/insights/InsightsPanel.tsx`) with `scroll-mt-20` to clear the sticky nav, applied in **every** branch of all three components — the empty branch especially, since Proof of lift ships empty for weeks at beta launch and that is exactly when a landing-page visitor would follow the link. Plus a "jump to" row under the h1 and both landing pages linking `/app/insights#proof-of-lift`. **Native hash scrolling does not work here** and a `useEffect` keyed on `useLocation().hash` + query-settled is required: the panels mount only after `insightsQuery` resolves, so the target element does not exist at navigation time. Revisit routes only if Issue 28 friend feedback shows people hunting for these by name.

**Two supporting changes.** `buttonVariants` moved to its own module (`components/ui/buttonVariants.ts`) and is exported so a router `<Link>` can wear the button skin — the repo's prevailing `<Link><Button/></Link>` idiom nests a real `<button>` inside an `<a>`, which confuses assistive tech and swallows keyboard activation, and a component adopted at 13 call sites should not multiply that. Own module because `button.tsx` must export only a component (`react-refresh/only-export-components`), matching the convention already documented in `components/chip/poses.ts`. Second, `components/insights/InsightsNarrative.tsx` had one branch covering `q7.isError || q28.isError || !metrics_available` — a failed fetch rendering as "not enough analytics data yet", which is the exact inversion the Issue-361 sweep exists to prevent ("a creator whose data exists would be told they have none"). Split into `QueryErrorState` with retry, and an `EmptyStatePrompt` for the genuine empty case.

**The reusable primitive.** `components/EmptyStatePrompt.tsx` types its action as a **required** discriminated union — `{actionLabel, actionTo}` | `{actionLabel, onAction}` | `{action: ReactNode}` — so a prose-only empty state no longer type-checks. That is the mechanism, not the styling: the walkthrough found ~9 empty states naming a prerequisite the creator could not click, several of them (`AnalyticsPanel`'s "connect your channel and sync") naming a control that does not exist anywhere in the app. 13 call sites adopted it.

**Verification.** Frontend `npx vitest run` — **354 passed / 55 files** (317→354 is +37 over the session; +17 on this issue), `npx tsc -b` clean, `npm run lint` 0 errors (3 pre-existing warnings), `npm run build` clean. Backend unchanged and re-measured: **2480 passed, 64 skipped, 173 deselected** — identical to the pre-change baseline, as expected for a zero-backend-change issue. Layer 0 all green: ruff 0 · mypy 0 · coverage 83.51 · module floors all pass · bandit 0/0 · pip_audit 0. **Date:** 2026-07-31

2026-08-04 — **Lane L26 declared: A→B Auto-Clipping MVP (Issues 414–426). L25 Batch C PAUSED. Ten plan-time decisions locked after a three-track research/design pass (full plan in the 2026-08-04 session; issue briefs in `docs/issues.md` § L26).** User decision on scope: the MVP viability criterion is best-in-class auto-clipping; user explicitly chose (a) speaker-aware pan+cuts over pan-only and over TalkNet-class AV detection, (b) pipeline auto-metadata for ALL clips over shortlist-only, (c) unifying Review+Editor around one stage over a Review-only redesign.

**(1) Batch C reorder.** L25 Batch C (393–397, 401) pauses; L26 runs first. #394 re-targets the ShortStage `overlay` slot; #396 becomes the manual override layered on the *computed* track AFTER auto-reframe ships (inverting its old premise); #399 unaffected. Trigger: the three verified gaps (no whole-video pass, static crop, 9%-of-workspace player) block the MVP criterion directly, and Batch C's items either depend on L26 surfaces or rank behind them.

**(2) Whole-video context pass — chain member, 1:1 table, never-fails.** New `analyze_video_context` inserted between transcribe and build_signals (deterministic availability for generate_clips; no parallel-fork race), storing validated `context_jsonb` in a new `VideoContext` table (0044 RLS pattern; rejected: column on the hot `videos` row). The task cannot raise — LLM/parse failure degrades to signal-only clip generation (today's behavior), guarded by `VIDEO_CONTEXT_ENABLED`. One call, no chunking: a 120-min transcript ≈ 26K tokens is well inside Sonnet 4-6's window; over ~4h transcripts are coarsened per-paragraph (chapters.py precedent), never truncated. Cost ≈ $0.07–0.11/video, +15–40s serial.

**(3) Hybrid candidate merge — the engine change the 2026-07-30 #377 entry required a CHECK for.** LLM-proposed moments (≤4, validated, sentence-snapped) are UNIONED with signal candidates under signal-priority NMS pre-scoring; displacement happens post-scoring on merit via the post-ranking trim to `CLIPS_PER_VIDEO_DEFAULT=8`. `extract_candidates` is byte-untouched, so the eval harness stays green by construction; 3 new merge scenarios raise `SCENARIO_FLOOR` 15→18. Industry check: hybrid signal+semantic candidate generation with NMS union is the standard highlight-detection composition (SumMe/TVSum lineage already cited at `candidates.py:297`); an LLM-only proposer was rejected (unbounded geometry risk, harness unverifiable), heuristic-only rejected (cannot see flat-energy stories — the user's core complaint). Cold-start rule: llm-origin candidates score `max(_signal_score, 0.8·llm_confidence)` so a flat-energy story survives cold-start instead of scoring ~0. Also fixes `_PRINCIPLES` 11→12 (Clean Context Boundary was uncitable).

**(4) suggested_* vs applied_* metadata semantics.** The pipeline writes only `suggested_title/description/hook` (new 0054 columns, pre-clamped to YouTube limits); `applied_*` remains creator-typed only, set via the existing PATCH. Publish fallback order: `applied_* or suggested_* or (video.title | "#Shorts")`. One batched structured-output call per video for all clips (≈$0.03–0.04), enqueued parallel with render, fill-only on redelivery (`suggested_title IS NULL`). No disclaimer persisted — disclaimers belong to suggestion UIs and stay Python-appended on the on-demand (now "Regenerate") endpoints. Also fixes the clip-titles grounding bug (whole-transcript-truncated-to-1500-chars → the clip's own window).

**(5) Diarization: Deepgram `diarize=True` on pinned SDK 3.7.7.** Field verified present in the 3.7.7 tag source (the `words=True` outage precedent makes this check mandatory); the bare boolean is deprecated upstream in favor of `diarize_model` but works and routes to the v1 diarizer — upgrade noted for the next SDK bump. Schema addition is ADDITIVE (`speaker`/`speaker_confidence` on words, `speaker` on segments, keys omitted when absent); AssemblyAI gets parity via `speaker_labels`; WhisperX deliberately excluded (pyannote = heavy dep + HF token; the reframe ladder treats "no speaker" as a defined input). Kill-switch: `TRANSCRIPTION_DIARIZE_ENABLED`.

**(6) Speaker→face mapping WITHOUT lip-sync ML.** Co-occurrence + largest-during-turn + mouth-motion-energy voting (BlazeFace mouth keypoint patch diff on frames we already decode) with margin-ratio confidence, over a TalkNet-class audio-visual model — explicitly rejected as overkill for the ≤100-user beta (new ML surface, real latency/cost; consistent with the Issue-189 build-not-buy entry which deferred ASD). Fallback ladder `speaker_cut → face_pan → static` is the single source of truth; every ambiguity rung degrades toward today's behavior, never past it; off-screen speakers hold the current framing (never cut to nothing).

**(7) Shot detection: ffmpeg `scdet`, not PySceneDetect.** Amends the 2026-06-23 entry's "PySceneDetect when needed": zero new Python dependency (worker already has ffmpeg + the measurement-pass subprocess pattern), frame-accurate vs ±200ms histogram sampling, and we only need hard cuts in talking-head footage — PySceneDetect's edge (gradual transitions) is not needed and remains the documented upgrade path.

**(8) Crop-track persistence: new `clips.reframe_track_jsonb` column (0055), NOT ClipEditDocument.** The edit document is user-authored state under CAS revisions; a worker writing system output there would bump revisions and invalidate client autosaves. #396's own wording — manual override "layered over the computed reframe track" — implies two layers, two homes. Unified wire contract at `GET /clips/{id}/crop-track`: keyframes carry the clamped LEFT-edge x (the exact sendcmd values — one geometry definition shared by render and preview), cuts as explicit jump markers, 404 `no_crop_track` for pre-pipeline clips; re-render replaces or deletes, never stale. Track recomputed every render (persisted row = last render's track; no cache-invalidation machinery).

**(9) Review+Editor unification (ShortStage) + sizing inversion.** One shared stage component owning the 9:16 frame (player, placeholder states, meta row, cleaned-preview tab swap, overlay/below slots); pages keep what argues about or acts on the clip. Sizing inverts `toolLayout.ts`'s chrome-subtraction (the documented re-measure trap) to container-query-based: the grid gives the stage cell the height budget, the stage derives media size from its own cell (`STAGE_CELL` + `STAGE_MEDIA_W`). Measured target: Review media box ≈273×486 → ≈382×679 (~2.0× area) at 1440×900; Editor gains ~1.15× (Timeline dock is load-bearing per #390 and does not shrink). WhyThisClip splits into ClipCase + ClipMetadataPanel (title+hook = one truncating row each — the user's "phenomenal but too big" verdict operationalized). Crop preview ships staged: honest overlay mini-map now (renders nothing when no track exists); source-framing view (16:9 stream + animated crop rect) filed as follow-up — `GET /videos/{id}/stream` already exists, but `source_expired` + trim time-mapping need their own issue.

**(10) Issue-189 flag-flip is PENDING EVIDENCE, not decided here.** Enabling `ACTIVE_SPEAKER_REFRAME_ENABLED` in prod requires closing all four 189 unlock criteria on staging (image build, real 2-speaker visual verification, timing budget, tmp cleanup) — Issue 422 owns that checklist and the reversal entry lands there with the evidence, not before.

**Cross-track contracts (binding):** migrations assigned in land order 0053 video_context / 0054 suggested_metadata / 0055 reframe_track off head 0052; transcript `speaker` is optional-never-required for all consumers; new SSE stage labels `video_context`/`clip_metadata` are non-terminal and the frontend consumer must tolerate unknown labels; Track A merges first (it changes the chain shape), B rebases on A, C last. **Date:** 2026-08-04

2026-08-04 — **Issue 418: Deepgram diarization carries a $0.0020/min surcharge — `COST_PER_MIN_DEEPGRAM` 0.0077 → 0.0097.** The L26 plan (decision 5) required verifying whether `diarize=true` changes the Deepgram bill. Verified against the primary source (deepgram.com/pricing, fetched 2026-08-04): Nova-3 pre-recorded is $0.0077/min base and speaker diarization is a **$0.0020/min add-on**, so the all-in pay-as-you-go rate with diarization enabled is **$0.0097/min**. Since `TRANSCRIPTION_DIARIZE_ENABLED` defaults ON, the price-book constant now carries the all-in figure (the honest number for spend estimates), `PRICE_BOOK_VERSION` bumped to `2026-08-04`, and the pinned rate tests in `tests/test_usage_ledger.py` updated with the new arithmetic. A separate surcharge constant keyed off the kill-switch was rejected as premature — the flag is expected to stay on, and the price book is an estimation surface, not an invoice. **Date:** 2026-08-04

2026-08-04 — **L26 Track C build deviations + merge notes.** Six build-time deviations from the Track C plan (Issues 423–426, branch `lane/l26-stage`), recorded at the L26 serial merge: **(1)** AppliedTitleField gained additive optional autoEdit/onClose props (default behavior unchanged; AppliedDescriptionField same shape). **(2)** sizing budget excludes the trim filmstrip — at 1440×900 it sits just below the fold inside the scrollable stage cell; including it would cap media at ~1.45× and fail the 1.8× gate (measured chrome constant 6.92rem → 7rem; Review 1.89×, Editor ~1.05× vs ~1.15× plan estimate — toolbar merge bought ~30px not 50). **(3)** CropTrackOverlay mounts inside ShortStage, overlay slot stays empty as the #394/#396 seam. **(4)** VideoClipsMap detail panel renders ClipCase (imported the deleted WhyThisClip). **(5)** Editor player shadow-accent-glow dropped for stage uniformity; Card stamps data-elevation as the one-primary-panel test hook; Editor xl sub-line removed by toolbar merge. **(6)** cleaned-preview tab swap shipped in-stage as planned. **Merge notes:** migration 0055 re-parented onto 0054 (down_revision `"0052"` → `"0054"`, single alembic head verified 0052→0053→0054→0055); the combined ClipOut surface (origin + suggested_* + has_crop_track) required ONE snapshot-style test update, taken at the merge (the pinned 0055-structure test now asserts the final parent). **Date:** 2026-08-04

2026-08-05 — **Issue 395: presigned direct-to-R2 multipart upload — architecture rulings, security posture, and two prod-incident-driven additions.** Trigger: the user's live stress test (2026-08-05) hit every failure mode the issue predicted — a ~40-min single-stream upload through the cloudflared tunnel, a Cloudflare-edge 403 at completion that the app never even logged, and a session that expired mid-upload (`/auth/me` 401 at 13:23). Live research pinned the R2 constraints (developers.cloudflare.com/r2: presigned-urls, buckets/cors, multipart-objects — fetched 2026-08-05). Decisions:

**(1) Stateless in-flight uploads — no table, no enum state, no migration.** Per-creator isolation is enforced by key-shape validation on EVERY sign/list/complete/abort call (`_validate_upload_key`: `source/{creator.id}/{yt-id|uuid4-hex}{whitelisted suffix}`, 403 on mismatch — a signature-issuance refusal, deliberately not 404-masked); R2 itself binds uploadId→key. The Video row is created only at /complete, reusing the legacy endpoint's dedupe → insert → IntegrityError-409 → stamp-before-start machinery. Rejected: an upload_sessions table / `IngestStatus.uploading` state (migration + frontend states for zero acceptance value). Abandonment GC = R2's **built-in 7-day auto-abort of incomplete multipart uploads** — documented in COMPLIANCE instead of building a sweeper.
**(2) Complete-endpoint ordering is load-bearing.** Dedupe (409) and balance (402) run BEFORE CompleteMultipartUpload so a rejection leaves the multipart session intact — after a top-up the client re-calls complete without re-uploading gigabytes. NoSuchUpload + an existing row for the same object → idempotent re-return (publications confirm-shape). **Deviation from the design agent's draft:** the IntegrityError race 409s WITHOUT deleting the object — an associated upload's key is deterministic (token = the YouTube id), so the winning row may point at this exact object; deleting would break the winner. Orphans are swept by the existing `source/` lifecycle rule.
**(3) Size enforcement without POST policies.** R2 does not support S3 POST policies (no `content-length-range`), so the cap is: declared `size_bytes` rejected at presign-time + HEAD verification at complete (oversize → delete + 413). `UPLOAD_MAX_MB` is re-scoped to the proxy paths only (dev fallback + OBS `/clips/ingest`, which stays — API-key CLI clients can't run Uppy); the direct path's real gate is the minutes quota plus new `UPLOAD_MAX_FILE_GB=20`.
**(4) CSP `connect-src` widened to the account-scoped R2 origin** — explicit reversal of the keep-R2-out-of-CSP stance (routers/videos.py poster-proxy rationale): browsers gate fetch PUTs on connect-src, which previously fell through to `default-src 'self'` and would block every part PUT. `img-src` stays `'self'`. Structural test added (test_static.py).
**(5) Advisory client duration, never persisted.** Create/complete accept HTMLVideoElement-read `duration_s` for the 402 pre-check UX only; the row is created `duration_s=None` and the worker's ffprobe at ingest is authoritative (a lying client cannot underpay — `deduct_for_video` uses the probed value). One-hunk worker change: filling an unset duration also reclassifies `kind`.
**(6) Uppy headless (@uppy/core+aws-s3+xhr-upload+golden-retriever v5), custom UI.** Uppy Dashboard CSS rejected (design-token/no-glyph structural gates + bespoke design system). ONE UI, two transports: `GET /videos/uploads/config` picks `multipart` (r2) or `proxy` (local dev → legacy endpoint), so dev needs no R2. Per-part sign endpoint (1200/min) over batch signing: matches Uppy's signPart contract exactly and presigning is local-only (~1ms); a 20 GB gigabit worst case is ~350 signs/min. Fixed 25 MiB `getChunkSize` pins R2's all-parts-equal-except-last rule (10,000×25 MiB ≈ 244 GB headroom). GoldenRetriever is guarded behind `localStorage`+`indexedDB` presence (private-browsing modes) and its service worker ships as a classic-script copy at `public/uppy-sw.js` (the shipped file's `export {}` marker can't register as a classic worker — keep in sync on upgrades). Resume matrix: same-session drop → per-part retry; reload with SW → nothing re-sent; browser restart → ghost file, re-select resumes the same uploadId via listParts.
**(7) Session-expiry pause/resume (new requirement from the incident).** The 60-min JWT dies inside any long upload, and `api()`'s 401 redirect would navigate away and lose the queue — so the upload endpoints use `redirectOn401: false`; a 401 pauses the queue, surfaces "sign in again (new tab), then Retry — parts are kept 7 days", and retry resumes via listParts. A session-refresh endpoint was explicitly left as a separate product decision.
**(8) cloudflared tunnel → HTTP/2** (`TUNNEL_TRANSPORT_PROTOCOL: http2` in docker-compose.prod.yml): Cloudflare-confirmed QUIC underperforms HTTP/2 (cloudflared#895); upload bytes now bypass the tunnel entirely, this carries API/SSE/legacy traffic. Also: the 100 MB Cloudflare proxy body cap (Free AND Pro — user's zone is Pro) silently capped the legacy path below `UPLOAD_MAX_MB=500`; the direct path is structurally immune (r2.cloudflarestorage.com is not zone-proxied).
**(9) Bucket CORS via script, not dashboard:** `scripts/r2_set_cors.py` (PUT + Content-Type + **ExposeHeaders ETag** — without which multipart completes stall at 100% — + MaxAge 3600), run once per environment with a `get_bucket_cors` echo.
**(10) e2e stays on proxy mode** (mock-api serves `/videos/uploads/config` → proxy) so Playwright never emits traffic to the unmocked R2 origin.

**Verification.** Backend: full unit suite **2795 passed / 0 failed** (new tests/test_videos_multipart_upload.py — 27 tests: cross-creator/malformed-key 403s, 402-before-complete mock-ordering, oversize-HEAD delete+413, IntegrityError-no-delete, NoSuchUpload idempotency, ListParts pagination, part sorting; + 6 rate-limit qualnames + CSP connect-src test). Frontend: `tsc -b` clean, eslint 0 errors (1 pre-existing warning), vitest **637 passed / 88 files** on node 22 (624 → 637; the first run on this box tripped the documented node-26-breaks-jsdom gotcha — 35 phantom localStorage failures, re-verified harmless and re-logged with a structural-fix suggestion in OFF_COURSE_BUGS 2026-08-05). Live acceptance (>2 GB upload, resume drills, session-expiry drill, CORS propagation) is the deploy-gated Phase 4 checklist in docs/issues.md #395 — NOT yet run. **Date:** 2026-08-05

2026-08-12 — **Lane L29 Batch E authorized (the lane-convention `[DEC]` gate): eval/CI-integrity work proceeds in two tranches.** The L29 lane convention requires an explicit DECISIONS entry before any Batch E (476–482) work begins, because this batch changes what the project's own gates *mean*. Ruling: **(1) Tranche 1 — mechanics, authorized now:** Issue 479 (make ci.yml's per-module coverage floors + diff-cover actually evaluate — single-invocation the coverage job or move the `_coverage.xml` cleanup out of `main()`, plus "skipped"-fails-in-CI semantics, `ci_local.sh` unit-lane precondition fix, pre-push hook install) and Issue 477 (eval-runner strictness: reject unknown expectation keys, `max_candidates` support, unconditional setup-before-peak, one-to-one candidate matching, floor pinned in a second location). These harden existing gates without changing what they claim to measure; 479 lands first in W0 so every subsequent L29 PR is actually gated, 477 lands in W1 *before* Batch A adds fixtures (fix the measuring instrument before using it). **(2) Tranche 2 — designs, each requiring its own `[DEC]` at build time:** Issue 476 (LLM-scorer eval lane — goldens format + nightly behavioral design; deliberately decided AFTER W3's Issue-461 structured-output ruling, which defines the recorded-response format) and Issue 478 (real-media `render-env` lane design). Sequencing evidence: the 2026-08-12 audit (eval dimension COMPROMISED), Issue 479's live-CI-log proof that module floors/diff-cover have reported "skipped" since 2026-06-23, and the W2 plan validation that unconditional setup-before-peak breaks zero existing scenarios (all 15 geometry scenarios already satisfy it via the `candidates.py:364` clamp). **Date:** 2026-08-12

2026-08-12 — **Issue 448's live re-render drill: deliberately allowed to lapse with the 2026-08-13 19:23 UTC source purge; the live verification moves to the W6 fresh-upload back-test on the SAME recording.** The drill (re-render ranks 3/13 of video `7e988321` with `OVERLAY_BAND_DETECT_ENABLED=true`) was already deferred once by the owner; the audit then found Issue 466 — the overlay-band seek-sampler is broken on every source > ~500 s (lexicographic frame sort past 999 samples, complete-stack timestamp assumption, 240 s budget truncation) — so running the drill now on this 1617 s source would mis-mask and produce a false failure against a defect we already have filed. Owner decision (2026-08-12, plan approval): let the source purge; after Batch C (466 + 467) lands, the owner re-uploads the same original recording ("2026-08-05 07-59-55") and one fresh pipeline run serves as the consolidated live back-test for 466 (spans vs the frozen ground truth 885–914 s / 930–1006 s in `tests/fixtures/superchat/README.md`), 467 (punch-in end-to-end), 427 (frame check), 444 (idempotency drill), and the 448 re-render itself. **Preserved before lapse:** the pre-L29 DB snapshot (`docs/assessment/exports/7e988321_pre_l29_snapshot.json`) and R2 HEAD-verification of all 11 "before" render artifacts, which survive the purge (`purge_stale_source_media` deletes source + WAV only). **Contingency:** this decision assumes the owner still holds the original recording file locally — confirmation requested in W0; if the file is gone, the fallback is that 466/467 verification rests on the synthetic-source integration tests plus frozen fixtures alone (weaker but sufficient for the code-level acceptance boxes; only the live-drill checkboxes would be closed as not-reproducible). **Date:** 2026-08-12

2026-08-13 — **W2 Batch A (Issues 457/458/459/460): four selection-geometry rulings in `clip_engine/` + `ingestion/audio.py`.**
**(1) De-overlap precedence (457):** one event class per audio sample. At emission, laughter (the stricter detector: energy + spectral ZCR) wins — frames it claims are excluded from the energy mask. At consumption, `build_signal_array` accumulates laughter and energy in separate arrays and adds `np.maximum(laugh, energy)` into the composite (defense-in-depth for already-persisted timelines and hand-authored fixtures); retention stays additive, silence subtractive. Summing stacked up to 3.5× weight on the same reaction samples, systematically biasing peaks toward the aftermath. Mutually-exclusive event classes are the standard sound-event-detection framing (DCASE: https://dcase.community/challenge2016/task-sound-event-detection-in-synthetic-audio, fetched 2026-08-12).
**(2) Positive-evidence floor (458):** `_PEAK_MIN_HEIGHT = 0.25` passed as `height=` alongside `prominence=0.5` — scipy ANDs absolute height with relative prominence (https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.find_peaks.html, fetched 2026-08-12). Prominence alone is relative: a flat 0 stretch between two −0.5 silences carried prominence 0.5 and fabricated candidates from nothing. 0.25 validated against the post-457 fixture survey (weakest genuine peak: 1.35). New skip reason `no_positive_signal` checked before the retention branch, with honest label copy. **The Issue-217 skip-reason taxonomy is now five strings:** no_signal_above_threshold, no_positive_signal, insufficient_retention_data, source_unavailable, all_candidates_suppressed_by_nms — priority order documented in `derive_skip_reason`.
**(3) Endpoint pad + inward clamp (459):** `_detect_peaks` pads the composite one sample per side with `min(0.0, signal.min())` — never −inf, which corrupts prominence ordering — and shifts indices back; an endpoint peak is nudged one sample inward (idx 0 → times[1]; idx n−1 → times[n−2]) so setup < peak < end hold. scipy find_peaks definitionally never reports endpoints; caller-side padding is the standard workaround.
**(4) Nearest-spike ruling (460):** the setup fallback's `min()` (earliest spike) lost 2-docstrings-vs-1-line — the module contract and function docstring both promise most-recent; `max()` now mirrors the silence rule.
**Evidence:** red-first proofs recorded in the four commit messages on `fix/issue-457-460-window-signals`; 4 new eval fixtures; full unit lane + eval suite green with zero pre-existing expectation changes. **Date:** 2026-08-13

2026-08-13 — **Issues 456/449: the never-mid-sentence start guard is BOUNDED, not absolute (amends the 2026-08-05 Issue-428 ruling r.(2)).**
The 2026-08-05 decision declared the start guard *absolute* — a start strictly inside a sentence is always moved to a sentence start, even beyond `SENTENCE_SNAP_MAX_S`. The 2026-08-12 audit (Issue 456, SEV1) proved the absolute rule can drag a start so far back that the 90 s clamp cuts the clip BEFORE its own peak (shipped `[204.7, 294.7]` excluding peak 300), and the Deepgram no-utterance fallback collapsed whole videos to one `[0, 90]` clip. The rule is now bounded in `clip_engine/sentence_snap.py` by (1) caller-supplied `backward_limit_s = peak_s − (CLIP_TARGET_MAX_S − _MIN_PAYOFF_TAIL_S)` with `_MIN_PAYOFF_TAIL_S = 5.0` (peak−85 s — any earlier start makes even the clamp ceiling land < 5 s past the peak), and (2) `_RUN_ON_BACKWARD_CAP_S = 30.0` (= MIN_CLIP_S: a "sentence" needing more drag than a whole minimum clip is punctuation failure, not grammar). Past either bound with forward unusable, the RAW start is kept — a mid-sentence open is recoverable in review; a clip without its peak is not. A post-snap invariant `setup+0.1 ≤ peak ≤ end−0.1` is enforced on every returned candidate: REPAIR first (restore the pre-snap raw end, clamped to min(duration, setup+90)), then DROP with a WARNING as the unconditional backstop. A degenerate sentence index (single unterminated span covering ≥ 80 % of the video) disables snapping entirely with one WARNING per video — the engine-side log leg Issue 481 builds on. **Issue 449 (same module):** the pause branch now tests the first sentence at/after the start — what the clip audibly opens on — instead of an exact-coincidence epsilon; strong openers stay untouched (never snapped forward), weak openers take the existing Issue-441 bounded walk-back (the live 1306.43 → 1301.87). The r.(1) word-level tripwire (extract_candidates/snap_to_sentence_boundary byte-identical), the 2026-08-07 NMS-before-snap ordering, and D.1 containment threshold are all untouched.
**Source/evidence:** repro rerun in `docs/issues.md` §456; `docs/assessment/CLIPPING_INTEGRITY_2026-08-12.md`; failing-first tests in commit 1c9bf72. Industry practice: bounded operations with enforced invariants and explicit recovery (NASA/JPL Power of Ten rule 5, https://spinroot.com/gerard/pdf/P10.pdf); clip-vs-remove as first-class geometry-invariant responses (https://docs.pytorch.org/vision/stable/ops.html); production captioning bounds boundary snapping by distance and otherwise leaves the cut alone (https://partnerhelp.netflixstudios.com/hc/en-us/articles/360051554394). All fetched 2026-08-12. **Date:** 2026-08-13

2026-08-13 — **Scoring adopts structured outputs (Issue 461; supersedes the 2026-06-29 Track-B deferral).** The 2026-06-29 entry deliberately deferred `output_config` on `clip_engine/scoring.py` ("core ranking path — defer the schema"). Adopted now, owner-ruled: (1) Issue 461 showed extraction-only parsing left three trust defects in the one function that decides what ships; (2) Issue 476 needs a pinned golden response format before goldens are recorded. Shape: root-object wrapper `{"scores": [...]}` (structured outputs require an object root); per-item `{index:int, dna_score:number, score:number, principle:enum, reasoning:string}`, `additionalProperties: false` throughout; the principle enum is BUILT FROM `_PRINCIPLES` (clip_explain precedent) so registry additions cannot desync the schema. `extract_json_block` + a per-item validator (int()-coerced index, guarded float coercion → per-item cold-start, registry-checked principle → safe default) remain as defense-in-depth because refusal/max_tokens responses are not schema-guaranteed, and numeric clamps stay in code (json_schema cannot express minimum/maximum). One-time cache invalidation of the static prefix accepted (Issue 416 precedent). SDK: pinned `anthropic==0.105.2` supports `output_config` natively (signature-verified; same call shape as `analysis/video_context.py`). Source: https://platform.claude.com/docs/en/build-with-claude/structured-outputs (fetched 2026-08-12). **Date:** 2026-08-13

2026-08-13 — **Issue 463: video_context/clip_metadata prompt placement aligned with the Issue-224 rules.** (1) `analysis/video_context.py`: creator-authored identity_text moved from the system role to the user turn via `wrap_untrusted('creator_stated_identity', …)` ahead of the transcript wrap; the cached block 2 is now DNA-only via `knowledge.util.dna_system_block` (floor-gated 1h marker), PROMPT_VERSION 2→3. (2) `knowledge/clip_metadata.py`: system Block 3 (the model-authored context summary — DERIVED from the untrusted transcript, a second-order injection surface per OWASP LLM01 Scenario #4) deleted; the summary rides `wrap_untrusted('video_context_summary', …)` at the head of the user turn; system pinned at exactly [static, DNA], byte-identical regardless of summary. tool_result wrapping re-confirmed N/A (single-shot generation calls) against https://platform.claude.com/docs/en/test-and-evaluate/strengthen-guardrails/mitigate-jailbreaks + https://genai.owasp.org/llmrisk/llm01-prompt-injection/ (both fetched 2026-08-12). Cache re-baseline: block-2 bytes changed → one-time cold 1h write per creator, self-healing; thereafter identity edits no longer invalidate the cache (CI byte-identity pin) and the per-video summary can no longer break the DNA breakpoint. The scorer's third system block (Issue-371 ruling) explicitly untouched. The "wrapped at every downstream site" acceptance closed by verification: the only other consumer is the scorer's llm_reason leg, already wrapped at `clip_engine/scoring.py:347-350`. **Date:** 2026-08-13

2026-08-13 — **Issue 465 score-semantics ruling: `clips.score` is the IMMUTABLE DNA/LLM fit composite.** Set once at persist and never rewritten; the personalization blend `(1−w)·score + w·pref` moved to new `clips.blended_score` (migration 0059, Template A expand-only) written solely by `rerank_with_preference`, which sorts/ranks by it; NULL = personalization never applied. **NO backfill, deliberately:** pre-0059 reranked rows had their original fit destroyed by the overwrite — the blend cannot be un-mixed, so there is nothing correct to copy; those rows may carry a historical blend in `score`, contamination bounded by (1−w) ≥ 1−PREFERENCE_WEIGHT_CAP and washing out as clips regenerate. The registered `append_ranked_clips` mixed-column item is resolved: append rows are now uniform with the initial persist (score=fit, blended=NULL — append still skips the rerank per the 2026-08-05 ruling). The blend formula was extracted to `preference.model.blend_scores`, shared by the rerank and `preference/efficacy.py` — this IS Issue 475's blend-parity interface (pinned by test). Side repair with zero code change: proof-of-lift's contrast average and efficacy's `dna_composite` were silently ingesting blended values for ranks 1–12; both now read true fit. **Date:** 2026-08-13

2026-08-13 — **Issue 464 (Principle 10 "Native length"), owner ruled OPTION A: wire it, don't descope.** Estimator: median `avg_view_duration_s` over the creator's top SHORTS only (mixing long-form watch durations — minutes-scale — was why the stored value could never drive geometry); no metered Shorts → None. Wiring: a single per-creator ceiling `effective_max_len_s = clamp(optimal × 1.25, 45.0 s, 90.0 s)` derived once in `score_and_rank` and threaded to the sentence-snap length clamp and the LLM-window clamp; 1.25 headroom lets the sentence-aligned cut finish a thought, 45 s floor keeps the platform-viable Short band, 90 s ceiling is the existing `CLIP_TARGET_MAX_S`. **Max-clamp ONLY** — windows are never stretched toward the ceiling; `MIN_CLIP_S` and `POST_PEAK_S` untouched. Cold start (no DNA / no Shorts / stale long-biased rows) resolves to the platform constant — byte-identical pre-464 geometry, proven by the untouched 31-scenario eval suite; stale mixed-median prod values clamp harmlessly to 90 s until the next DNA build (no backfill). External evidence: no universal optimal length exists and per-account analytics beat benchmarks (https://support.google.com/youtube/answer/10059070 · https://www.socialinsider.io/blog/how-long-are-tiktok-videos/ · https://vidiq.com/blog/post/youtube-shorts-algorithm/, fetched 2026-08-12). `docs/CLIPPING_PRINCIPLES.md` Principle 10 reworded to the operative contract. **Date:** 2026-08-13

2026-08-13 — **W4 render-lane rulings (Issues 467/469/478 legs 1–2).**
**(1) Punch-in mechanism (467):** animated `scale` with `eval=frame` (w/h expressions re-evaluated per incoming frame, `t` available only under eval=frame — https://ffmpeg.org/ffmpeg-filters.html#scale-1) + a CONSTANT centered crop. The Issue-184 chain put the time expression in crop w/h, which ffmpeg evaluates ONCE at filter configuration where t=NaN (https://ffmpeg.org/ffmpeg-filters.html#crop) — every zoom_on_peak render failed rc=234; the 5.1.6 texi documents identical semantics, so the fix is version-stable across prod 5.1 / CI 6.1 / dev 8.1. `zoompan` rejected: built for stills (`d` holds each input frame for d output frames) and forces a fixed output fps that re-clocks the stream (https://ffmpeg.org/ffmpeg-filters.html#zoompan). `trunc(*/2)*2` keeps libx264-even dims; the trailing crop is unlabeled so sendcmd `crop@spk` can never address it.
**(2) Duration authority (469):** the ingest ffprobe container duration is the ONE authority — `_apply_probed_duration` ALWAYS overwrites `duration_s` + `kind` on probe success (the YouTube-API integer seed was never corrected before); `container_duration_s` threads generate_clips → score_and_rank → extract/snap with the end clamp INSIDE the W2 invariant tail (payoff-cutting clamps hit repair-or-drop); render demotes sub-second overshoot via `_DURATION_OVERSHOOT_EPS_S = 1.0` (clamp + warn < 1.0 s, terminal ≥ 1.0 s — container durations can be estimates, AVFMT_DURATION_FROM_* per https://ffmpeg.org/doxygen/trunk/structAVFormatContext.html); > 4 h sources clip only from the first 14400 s (stated at the constant).
**(3) Filter-config terminal classification + marker rename (467/478):** `_run` maps four deterministic stderr signatures ('Error when evaluating the expression', 'Error reinitializing filters', 'Error initializing filter', 'No such filter') to ValueError = permanent in render_clip — no more 3-retry burn on unfixable argv. `render-env` → `render_env` because a hyphenated marker can never be applied as a decorator (`@pytest.mark.render-env` is a SyntaxError) — the defect that kept the real-media lane empty; addopts now excludes it and a HARD CI step (unconditional ffmpeg apt install — NOT preinstalled on ubuntu-latest per the runner-images manifest — plus a ≥3-collected floor) runs it. **Date:** 2026-08-13

2026-08-13 — **Issues 468/447: confirm/discard CAS + download tracking as a column.**
**(1)** Download tracking is a COLUMN (`clips.downloaded_at`, migration 0060), not an inference — ClipPublication records publishes not downloads; event_logs purges on the 90-day cycle so inference goes amnesiac; the download endpoint is dual-purpose (disposition=inline backs the in-app player), so the stamp writes ONLY for disposition=attachment AND variant=original, and clears on re-render (Issue-353 reset, restored if the enqueue fails) and on the clean/confirm swap — both replace the render the stamp described.
**(2)** Confirm/discard race-safety via conditional UPDATE (compare-and-swap: WHERE re-checks the value this request read, rowcount-checked) rather than `with_for_update()` — unit-lane test ergonomics (a lost race is rowcount=0), no lock-waits, identical under RLS; rowcount 0 → 200 noop, and discard's noop path never touches storage.
**(3)** Discard purge guard: skip the delete when the pending artifact URI equals `render_uri` — a double clean writes the same `_clean` key confirm already swapped in; deleting it destroyed the live render (found and red-proven during the build; OFF_COURSE 2026-08-13). The false 'R2 lifecycle prefix' claim in confirm's docstring corrected; the superseded original is now best-effort-deleted after the CAS commits (W4 integration, closing OFF_COURSE L143's storage leak). **Date:** 2026-08-13

2026-08-13 — **Issues 446 + 471: erasure enumeration, archive-vs-erase semantics.**
**Enumeration helper (worker/erasure.py):** media deletion enumerates DB URI columns ∪ deterministically CONSTRUCTED keys (clips/{id}.mp4, _clean, _edit, posters/{cid}/clip-{id}.jpg, audio/{vid}.wav, summaries/{sid}.mp4), deduped by extracted key. Reconstruction beats bookkeeping: the DB pointer to render keys is not stable (the clean-confirm swap orphans the original with no row referencing it — L143), every render key is a pure function of the row id, and deleting a never-written key is a free idempotent no-op (RFC 9110 §9.3.5 — https://www.rfc-editor.org/rfc/rfc9110.html#name-delete). `KEY_PATTERNS` is the grep-verified 10-entry write-site inventory with a sync-gating test.
**Archive vs erase + the posters/peaks default (owner ruling):** `DELETE /videos/{id}` = soft-delete — purges source/audio/render media, PRESERVES the row, clips, `clip_feedback` (training labels), and posters + peaks (the 'index entry' class COMPLIANCE already retains past the source purge; exactly what identifies an archived row for restore). `POST /videos/{id}/erase` = permanent (full enumeration incl. posters/peaks + recap summaries, then row cascade; UI names the training-data loss). Archive is a library state, not an erasure mechanism.
**Object-Lock posture:** per-URI deletes are independent, missing objects are success, refusals log-and-skip; callers null only successful pointers. Consequence: the source-retention beat deliberately does NOT filter `archived_at` — it is the retry belt that eventually frees a lock-refused blob; filtering would turn a transient refusal into a permanent retention violation.
**Exports-bundle note:** data_exports is one-row-per-creator and re-requests overwrite `export_uri`, orphaning the prior bundle — `erase_creator` now sweeps `exports/{creator_id}/` in addition to the enumerated current URI. The GDPR export excludes archived videos' rows from the video list but still includes their retained clips/feedback (Art. 15 honesty). Authority for derived-artifact completeness: EDPB one-stop-shop case digest on Art. 17 — https://www.edpb.europa.eu/system/files/documents/2023-02/one-stop-shop_case_digest_on_the_right_to_object_and_right_to_erasure_en.pdf. **Date:** 2026-08-13

2026-08-13 — **Issue 442: REMOVE the 'background' render style end-to-end (owner ruling).** The control never did anything — the ffmpeg chain is crop→scale full-bleed at every supported aspect, so there is never a letterbox to fill; the `_BACKGROUND_STYLES` blur graph was dead code, and the UI's "Default (black)" label was a FALSE claim (the default is a full-bleed crop, not a black fill) — a lying control in a product whose moat is honesty. Dropped from RenderStyleIn, the brand-kit models/merge, `style_learn._KIT_FIELDS` (load-bearing: the Issue-187 suggestion/accept loop writes suggested fields into the kit verbatim and historical `style_preset` rows still carry the key — a red test proved the suggester actively proposing the dead knob), both frontend panels, types, fixtures, and originality's style signature. No migration: stored JSONB keys silent-ignore by construction (legacy-row round-trip tested). Deliberate break: `extra='forbid'` now 422s background-bearing render POSTs — SPA and API deploy together. Industry evidence: competitors ship background fill only for CONTAIN/FIT layouts, not speaker crops (Opus "Fit" = opaque padding for screen-share, https://help.opus.pro/docs/article/layout-and-reframing; Descript horizontal-in-vertical embedding, https://www.descript.com/blog/article/how-to-make-horizontal-video-vertical). Successor: Issue 483 (contain/fit export mode, demand-gated). Rider (OFF_COURSE L148, closed): `doctor.py` now probes Stripe through `billing.stripe_client._STRIPE` and R2 through `worker.storage._r2` — a probe that bypasses the app's own client can green-light an outage, which is exactly what it did for 10 weeks. **Date:** 2026-08-13

2026-08-13 — **Issue 470: effective render geometry is PERSISTED, not derived.** Two `clips` columns (migration 0061, expand-only): `pending_geometry_jsonb` (worker-written in lockstep with `cleaned_render_uri` — the clean path's cut list never exists outside the task, and the Issue-391 edit-document clear at confirm kills the edit path's copy, so read-time derivation was impossible twice over) and `effective_geometry_jsonb` (confirm-written inside the Issue-468 CAS, composed across repeated trims, original clip-relative; `cleaned_render_uri`'s CAS equality doubles as the pending record's version token since they only change together). A legacy pending artifact (worker pre-0061) confirms with the record CLEARED plus a warning — honest fallback over a stale claim. **Re-render of a confirmed-clean clip stays AVAILABLE** (it is the only path to change caption style/aspect) **but discards honestly**: the Issue-353 reset clears the geometry in the same transaction, the 202 reports `discarded_edits: true`, `ClipOut.has_baked_edits` lets the UI warn before the click, and the 359c enqueue-failure path restores the record with the render. Duration, `/transcript`, and both trim validators read the delivered record (the validators previously accepted trims that could overrun the real file). **Date:** 2026-08-13

2026-08-13 — **Issue 472: feedback `skip` is an acknowledged no-op, never a verdict (W5).** The same `skip` token meant "advance past this clip" on the feedback surface but "retract your latest label" in the training partition, so the shipped Trim→Skip flow silently erased keep labels while the pile stayed `kept` (the 444 invariant violated — the audit's SEV1). Navigation and retraction are now different verbs: `POST /feedback action=skip` returns 201 with `id: null` and writes nothing (back-compat for stale SPAs); retraction is exclusively `PUT /triage → pending`. **Data repair (migration 0062):** deletes only MASKING skips — latest-verdict skips on kept/dropped clips, trailing runs removed whole — preserving superseded skips and pending-retraction skips; idempotent; the would-delete count logs at WARNING before the batched delete (alembic's NullHandler + a handler-less env.py swallow anything quieter). Telemetry note: `clip_feedback_submitted` no longer fires for skips — funnels counting them will flatline by design. **Date:** 2026-08-13

2026-08-13 — **Issue 473: retrain freshness — watermark on the full verdict set + outcome writes, plus the missing poller enqueue (W5).** The debounce watermark counted only TRAINABLE feedback rows, so retractions and `performed_well` arrivals never retrained — and no enqueue site for outcomes existed at all (the filed issue missed this; widening the watermark alone would have been dead code). Now: the watermark sees timestamps of the full `VERDICT_ACTIONS` set OR judged `ClipOutcome` writes (`fetched_at > model.updated_at AND performed_well IS NOT NULL`, joined via Clip), and `poll_clip_outcomes` enqueues the same countdown-coalesced `retrain_preference` per affected creator; `enqueue_retrain` moved to one shared home in `worker/tasks.py`. A monotonic sequence id was ruled out (schema change + backfill for no extra power at beta scale). Grounding: Google Rules of ML #8/#32/#37 — https://developers.google.com/machine-learning/guides/rules-of-ml (fetched 2026-08-13). Issues 474/475 ride the same lane: `active := weight > 0` in the one personalization-status producer, and `training_rows_select()` shared verbatim between training and the efficacy harness with `_relevance_for` action-first and the origin helper (`clip_origin_s`) adopted by lift + the originality fingerprint. **Date:** 2026-08-13

2026-08-13 — **[DEC] Issue 476 LLM-scorer eval design (Batch-E tranche 2, owner-approved).** Chosen: (1) **response-body goldens** — real recorded Anthropic bodies replayed via `anthropic.types.Message.model_validate` at the `_ANTHROPIC` boundary, driving the REAL `score_candidates` parse/annotate path in the unit lane; (2) **schema-hash forcing** — each golden pins sha256 of the canonical `_OUTPUT_SCHEMA` plus the scoring model id, so any schema or model change fails CI until a ~$0.20 re-record (a golden can never green-stamp a contract it wasn't recorded against); (3) **code-graded nightly behavioral lane** (~7 calls ≈ $1/night at Opus 5 rates) with three fixed assertion classes — setup-vs-aftermath ordering, strict shape/refusal detection, dna_score on/off-brief ordering — majority-of-3 on ordering probes, margins posted to the step summary. **NO LLM judge**: every criterion is an objective ordering/enum/range, where Anthropic's own eval guidance recommends code-graded assertions (https://platform.claude.com/docs/en/test-and-evaluate/develop-tests.md; consistent with https://developers.openai.com/api/docs/guides/evals). **NO VCR/cassettes**: request bytes churn with every prompt edit; body-goldens + the pin give the same regression power without cassette-invalidation noise. Proven live pre-merge: 3/3 green (margins +0.84–0.89) with an inverted-assertion red demonstrated. **Date:** 2026-08-13

2026-08-13 — **Issues 481 + 478 leg 3: transcription-fidelity fixture + real-path reframe proof (W5).** (1) Timing fixture = LibriSpeech test-clean utterance 1089-134691-0001 (CC BY 4.0, Panayotov/Povey et al., https://www.openslr.org/12), 5.4 s → 16 kHz mono WAV checked in; ground truth = corpus reference words + one recorded Deepgram nova-3 response (request options byte-matched to production), sanity-gated. Tolerance ±0.25 s — Deepgram publishes no timestamp SLA (https://deepgram.com/learn/working-with-timestamps-utterances-and-speaker-diarization-in-deepgram); below the smallest boundary-flipping gap, above observed re-run jitter; a constant sub-250 ms all-provider bias is an accepted blind spot. Parse fidelity is float-EQUAL per backend (pins the AAI ms÷1000 class; the AAI fixture is schema-derived — no key exists — labeled honestly; the WhisperX recorded leg is DESCOPED: ~2 GB torch for parse-fidelity is disproportionate). Owner-approved live nightly ASR leg rides the LLM nightly. (2) Degraded-transcript surface: additive `degraded: "no_utterances"` key in the stored JSON (omitted when healthy), optional field on the transcript endpoint, `transcript_degraded` SSE step — Deepgram-fallback-specific (AAI is single-segment by construction). (3) reframe_seats runs through the REAL BlazeFace path in `render_env` (CI provisions mediapipe + the pinned model, hard-fail; floor 3→7). **Scope narrowing recorded:** at production confidence the left face is undetectable in the 480 px frames (lower confidence produced phantom tracks and a WRONG mapping — rejected as ground truth), so the real-path proof covers speaker-seat geometry (median within 4.2 px of measured truth), live mouth signal, and mapping correctness; two-seat logic stays synthetically covered; a FULL-RESOLUTION re-freeze is queued for the W6 fresh upload. **Date:** 2026-08-13
