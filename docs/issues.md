...
### Issue 182: Export presets — 1:1 + 16:9 renders + clip download endpoint ✅ DONE (2026-06-22)
...
**Shipped:** `OUTPUT_PRESETS` registry in `render.py` (9:16/1:1/16:9), applied render-time via `style_preset["aspect"]` (no `ClipFormat` migration); crop width derives from the ratio so **9:16 is byte-identical**; punch-in + caption `PlayRes` (and lower-third margin) scale to the preset. Endpoint `GET /clips/{id}/download?variant=&disposition=` — per-creator 404 isolation, 302→presigned R2 (`attachment; filename=…`, 300s) in prod / `FileResponse` in dev; new `worker/storage.presigned_download_url`. Download button + aspect dropdown in the UI. **Also fixed a SEV2 off-course bug:** clip playback bound `<video>` to a raw `s3://` URI (no media endpoint existed) — now backed by the same endpoint with `disposition=inline`. Tests: +13 (`test_clip_download.py`, render presets, caption play-res). No DECISIONS entry (no PRD/scope change, per finding D0a/D0b). Note: the actual `/clips/...` mount has no `/api` prefix — endpoint is `GET /clips/{id}/download`.
