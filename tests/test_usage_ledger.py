...
def test_estimate_cost_deepgram_minutes() -> None:
    ...
    60 minutes of Nova-3 pre-recorded audio at $0.0097/min (base + diarization
    add-on, Issue 418) = $0.582 exactly. Pure-math assertion — no DB, no network.
    """
    minutes = 60.0
    expected_usd = minutes * settings.COST_PER_MIN_DEEPGRAM  # 60 * 0.0097 = 0.582
    assert abs(expected_usd - 0.582) < 1e-9, (
        f"Expected $0.582 for 60 min at ${settings.COST_PER_MIN_DEEPGRAM}/min, got ${expected_usd}"
    )
...
def test_estimate_cost_mixed_llm_and_deepgram() -> None:
    ...
    - 30 min Deepgram Nova-3 (+diarization) at $0.0097/min = $0.291
    Total expected: $4.791
    """
    tokens_in = 500_000
    tokens_out = 200_000
...
def test_nova3_price_and_version_pinned() -> None:
    """Issue 293 + Issue 418 — pin the nova-3 rate and its version stamp.

    Prod transcribes with nova-3 + diarization ($0.0077 base + $0.0020 diarize
    add-on = $0.0097/min, deepgram.com/pricing 2026-08-04); the old 0.0043
    nova-2 rate under-billed every transcription minute (Issue 293) and 0.0077
    omitted the diarization surcharge (Issue 418). A silent revert of either
    value must fail loudly here.
    """
    assert settings.COST_PER_MIN_DEEPGRAM == 0.0097
    assert settings.PRICE_BOOK_VERSION == "2026-08-04"
