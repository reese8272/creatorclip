...
def _python_source_files() -> list[Path]:
    """Collect first-party .py files only.

    Excludes tests/, the worktree-internal .claude/ tree, and — critically —
    virtualenv / build / cache trees (.venv, venv, site-packages, build, dist,
    *cache). Those hold third-party packages (some with deliberately non-UTF-8
    fixtures, e.g. joblib's encoding test) that are not our source and must not
    be scanned: rglob would otherwise descend into a local .venv and break the
    scan, even though CI / a fresh worktree (no venv at the root) passes.
    """
    root = _repo_root()
    excluded_dirs = {
        "tests",
