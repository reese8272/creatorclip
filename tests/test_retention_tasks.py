...
def test_purge_clears_source_uri_past_retention():
    ...
    # Issue 105 Fix 4: advisory lock is the first execute call. Returns True to allow body.
    ...
    async def fake_execute(stmt, params=None):
        ...
        # First non-lock call is SELECT, second is UPDATE.
...
def test_purge_continues_on_delete_error():
    ...
    # Issue 105 Fix 4: advisory lock is the first execute call.
    advisory_lock_result = MagicMock()
    advisory_lock_result.scalar_one = MagicMock(return_value=True)
    advisory_unlock_result = MagicMock()
...
def test_purge_filter_gates_on_ingest_done_at():
    ...
    # Issue 105 Fix 4: advisory lock result.
    advisory_lock_result = MagicMock()
    advisory_lock_result.scalar_one = MagicMock(return_value=True)
    ...
    async def capture(stmt, params=None):
        stmt_str = str(stmt)
        if "pg_try_advisory_lock" in stmt_str or "pg_advisory_unlock" in stmt_str:
            return advisory_lock_result
        captured["stmt"] = stmt
        return mock_result
...
def test_analytics_refresh_iterates_creators():
    ...
    # Issue 105 Fix 4: advisory lock and unlock results.
    advisory_lock_result = MagicMock()
    advisory_lock_result.scalar_one = MagicMock(return_value=True)
    advisory_unlock_result = MagicMock()
...
def test_analytics_refresh_orders_by_last_refreshed_nulls_first():
    ...
    # Issue 105 Fix 4: advisory lock result — captured but skipped when asserting ORDER BY.
    advisory_lock_result = MagicMock()
    advisory_lock_result.scalar_one = MagicMock(return_value=True)
